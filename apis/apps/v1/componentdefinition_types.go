...
type Action struct {
	...
	// Specifies whether the Action runs in non-blocking mode.
	//
	// When false, the Action runs in blocking mode and the call waits for completion.
	...
	// calls return the same in-progress or cached terminal result.
	//
	// This field cannot be updated.
	//
	// +kubebuilder:default=false
