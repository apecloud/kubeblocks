...
type ComponentDefinitionSpec struct {
	...
	//
	// +optional
	Configs []ComponentConfigTemplate `json:"configs,omitempty"`

	// Specifies groups of scripts, each provided via a ConfigMap, to be mounted as volumes into the container.
	// These scripts can be executed during container startup or via specific actions.
	//
	// Each script group is encapsulated in a ComponentScriptTemplate that includes:
	//
	...
	//
	// +optional
	Scripts []ComponentScriptTemplate `json:"scripts,omitempty"`

	// TODO(v1.0): parameters API

	// Defines the built-in metrics exporter container.
	//
	// +optional
	MetricExporter *MetricExporter `json:"metricExporter,omitempty"` // TODO(v1.0): remove this
	...
	//
	// +optional
	ServiceRefDeclarations []ServiceRefDeclaration `json:"serviceRefDeclarations,omitempty"` // TODO(v1.0)
}
...
type ComponentConfigTemplate struct {
	// Specifies the name of the config template.
	//
	// +kubebuilder:validation:Required
	...
	// Refers to the volume name of PodTemplate.
	...
	// The operator attempts to set default file permissions as 0444.
...
type ComponentScriptTemplate struct {
	...
	// The operator attempts to set default file permissions as 0555.
	// However, certain database engines may require different file permissions.
	// You can specify the desired file permissions here.
	//
	// Must be specified as an octal value between 0000 and 0777 (inclusive),
...
type SystemAccountStatement struct {
	// Specifies the statement template used to create the account.
	//
	// +kubebuilder:validation:Required
...
type SystemAccountPassword struct {
	// Specifies the policy for generating the account's password.
	//
	// This field is immutable once set.
	//
	// +optional
	GenerationPolicy *PasswordGenerationPolicy `json:"generationPolicy,omitempty"`

	// References to a secret from which data will be copied to create the new account.
	//
	// This field is immutable once set.
	//
	// +optional
...
// PasswordGenerationPolicy helps provide to customize complexity of password generation pattern.
...
type ComponentLifecycleActions struct {
	...
	// Defines the procedure to create a new account.
	//
	// Use Case:
	...
	//
	// +optional
	CreateAccount *Action `json:"createAccount,omitempty"`

	// Defines the procedure to delete an existed account.
	//
	// Note: This field is immutable once it has been set.
	//
	// +optional
	DeleteAccount *Action `json:"deleteAccount,omitempty"`
}
...
//   - `createAccount`: Defines the procedure to create a new account.
//   - `deleteAccount`: Defines the procedure to delete an existed account.
//
...
type Action struct {
	...
	// Specifies the gPRC request to perform.
	//
	// This field cannot be updated.
	//
	// +optional
...
// GRPCAction describes an Action that triggers gPRC requests.
type GRPCAction struct {
	...
	// Specifies the target port for the GRPC server.
	// It can be specified either as a numeric value in the range of 1 to 65535,
	// or as a named port that meets the IANA_SVC_NAME specification.
	//
	// +kubebuilder:validation:Required
	...
	// Indicates the gRPC service method to be invoked. Defaults to the action name.
	//
	// +optional
	Method string `json:"target,omitempty"`
...
// PreCondition defines the available pre-conditions to execute an action.
//
// +enum
// +kubebuilder:validation:Enum={Immediately,RuntimeReady,ComponentReady,ClusterReady}
type PreCondition string

const (
	ImmediatelyPreCondition    PreCondition = "Immediately"
	RuntimeReadyPreCondition   PreCondition = "RuntimeReady"
	ComponentReadyPreCondition PreCondition = "ComponentReady"
	ClusterReadyPreCondition   PreCondition = "ClusterReady"
)
