...
type ComponentDefinitionSpec struct {
	...
	// References to external config objects that the Component depends on.
	//
	// This field is immutable.
	//
	// +optional
	ConfigRefs []ConfigReferenceDecl `json:"configRefs,omitempty"`
...
// ConfigReferenceDecl represents the declaration of an external config reference.
type ConfigReferenceDecl struct {
	// The name of the external config reference.
	//
	// +kubebuilder:validation:Required
	...
	// The volume name can be used in podSpec.containers[*].volumeMounts.
	//
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:MaxLength=63
