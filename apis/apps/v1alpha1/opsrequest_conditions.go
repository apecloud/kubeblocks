...
func NewFailedCondition(ops *OpsRequest, err error) *metav1.Condition {
	msg := fmt.Sprintf("Failed to process OpsRequest: %s in cluster: %s, more detailed informations in status.components", ops.Name, ops.Spec.ClusterName)
	if err != nil {
		msg = err.Error()
	}
	return &metav1.Condition{
...
func NewRestartingCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to restart database in Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewInstancesRebuildingCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to rebuild the instances in Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewVerticalScalingCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to vertical scale resources in Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewHorizontalScalingCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to horizontal scale replicas in Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewVolumeExpandingCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to expand the volumes in Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewExposingCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to expose the services in Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewUpgradingCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to upgrade the version in Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewStopCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		Reason:             "StopStarted",
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to stop the Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewStartCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		Reason:             "StartCluster",
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start the Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewReconfigureCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		Message: fmt.Sprintf("Start to reconfigure in Cluster: %s, Component: %s",
			ops.Spec.ClusterName,
			getComponentName(ops.Spec)),
	}
}

func NewDataScriptCondition(ops *OpsRequest) *metav1.Condition {
	return newOpsCondition(ops, ConditionTypeDataScript, "DataScriptStarted", fmt.Sprintf("Start to execute data script in Cluster: %s", ops.Spec.ClusterName))
}
...
func NewReconfigureFailedCondition(ops *OpsRequest, err error) *metav1.Condition {
	...
	if err != nil {
		...
		msg = fmt.Sprintf("Failed to reconfigure: %s in cluster: %s", ops.Name, ops.Spec.ClusterName)
	}
	return &metav1.Condition{
...
func NewBackupCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to backup the Cluster: %s", ops.Spec.ClusterName),
	}
}
...
func NewRestoreCondition(ops *OpsRequest) *metav1.Condition {
	return &metav1.Condition{
		...
		LastTransitionTime: metav1.Now(),
		Message:            fmt.Sprintf("Start to restore the Cluster: %s", ops.Spec.ClusterName),
	}
}
