...
// validatePrimaryStsIndex check primaryStsIndex value cannot be larger than replicas
func (r *Cluster) validatePrimaryStsIndex(allErrs *field.ErrorList) error {
	for index, component := range r.Spec.Components {
		if component.PrimaryStsIndex != nil {
			if *component.PrimaryStsIndex > component.Replicas-1 {
				path := fmt.Sprintf("spec.components[%d].PrimaryStsIndex", index)
				*allErrs = append(*allErrs, field.Invalid(field.NewPath(path),
					nil, "PrimaryStsIndex cannot be larger than Replicas."))
			}
		}
	}
	return nil
...
func (r *Cluster) validate() error {
	...
	err = r.validatePrimaryStsIndex(&allErrs)
	if err != nil {
		return err
	}
