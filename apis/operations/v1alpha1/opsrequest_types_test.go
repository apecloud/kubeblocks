...
func TestValidateVolumeExpansionInstances(t *testing.T) {
	for _, sharding := range []bool{false, true} {
		for _, tc := range []struct {
			...
			noPVC, mixed, duplicateInstance, duplicateVolume bool
		}{
			{name: "mixed targets preserve override", instance: "custom", volume: "data", size: "6Gi", allow: true, valid: true, mixed: true},
			{name: "declared shrink without PVC", instance: "custom", volume: "data", size: "3Gi", allow: true, noPVC: true},
			{name: "declared expansion without PVC", instance: "custom", volume: "data", size: "6Gi", allow: true, valid: true, noPVC: true},
			{name: "duplicate template", instance: "custom", volume: "data", size: "6Gi", allow: true, duplicateInstance: true},
			{name: "duplicate volume", instance: "custom", volume: "data", size: "6Gi", allow: true, duplicateVolume: true},
			{name: "dotted template unsupported class", instance: "az.a", volume: "data", size: "3Gi"},
			...
			t.Run(tc.name+map[bool]string{false: "/component", true: "/sharding"}[sharding], func(t *testing.T) {
				...
				spec.Instances = append(spec.Instances, appsv1.InstanceTemplate{Name: "az.a"})
				...
				if tc.mixed {
					expansion.VolumeClaimTemplates = []OpsRequestVolumeClaimTemplate{{Name: "data", Storage: resource.MustParse("3Gi")}}
				}
				if tc.duplicateInstance {
					expansion.Instances = append(expansion.Instances, expansion.Instances[0])
				}
				if tc.duplicateVolume {
					expansion.Instances[0].VolumeClaimTemplates = append(expansion.Instances[0].VolumeClaimTemplates, expansion.Instances[0].VolumeClaimTemplates[0])
				}
				ops := &OpsRequest{ObjectMeta: metav1.ObjectMeta{Namespace: "default"}, Spec: OpsRequestSpec{Type: VolumeExpansionType, ClusterName: "test", SpecificOpsRequest: SpecificOpsRequest{VolumeExpansionList: []VolumeExpansion{expansion}}}}
				...
				for instance, size := range map[string]string{"": "1Gi", "inherit": "1Gi", "custom": "5Gi", "az.a": "1Gi"} {
					if tc.noPVC {
						continue
					}
