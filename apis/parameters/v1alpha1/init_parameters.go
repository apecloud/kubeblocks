...
type InitParameters map[string]InitParameter

// InitParameter describes the initialization overlay for one cluster sub-resource.
...
func (v InitParameters) Get(name string) *InitParameter {
	if len(v) == 0 {
		return nil
	}
	spec, ok := v[name]
