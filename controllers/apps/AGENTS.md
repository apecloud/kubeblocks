...
- `cluster/`: `Cluster` reconciliation, normalization, placement, sharding, TLS, service, restore intent injection, component status, and deletion flows.
- `component/`: `Component` reconciliation, workload generation, services, RBAC, accounts, variables, lifecycle hooks, TLS, validation, and status.
...
- Cluster restore intent is applied during cluster normalization and consumed by dataprotection/restore code. Keep intent annotation changes in `cluster/restore_intent.go` and test cross-namespace/default-namespace cases.
...
- For component changes, cover generated workloads/services/RBAC plus lifecycle, TLS, validation, status, and variable behavior when touched.
