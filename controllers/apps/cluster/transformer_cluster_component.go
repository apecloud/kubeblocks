...
func (t *clusterComponentTransformer) handleCreate(transCtx *clusterTransformContext, dag *graph.DAG, createSet sets.Set[string]) error {
        handler, err := newCompNShardingHandler(transCtx, createOp)
        if err != nil {
                return err
        }
...
func (t *clusterComponentTransformer) handleUpdate(transCtx *clusterTransformContext, dag *graph.DAG, updateSet sets.Set[string]) error {
        handler, err := newCompNShardingHandler(transCtx, updateOp)
        if err != nil {
                return err
        }
...
func deleteCompNShardingInOrder(transCtx *clusterTransformContext, dag *graph.DAG, deleteSet sets.Set[string], scaleIn *bool) error {
        handler, err := newCompNShardingHandler(transCtx, deleteOp)
        if err != nil {
                return err
        }
...
func definedOrders(transCtx *clusterTransformContext, op int) (appsv1.ClusterTopology, []string, error) {
        var (
                ...
                for _, topology := range clusterDef.Spec.Topologies {
                        if topology.Name == cluster.Spec.Topology {
                                if topology.Orders != nil {
                                        ...
                                        case updateOp:
                                                return topology, topology.Orders.Update, nil
                                        default:
                                                return appsv1.ClusterTopology{}, nil, fmt.Errorf("unknown operation: %d", op)
                                        }
                                }
                        }
                }
...
func newParallelHandler(op int) (clusterConditionalHandler, error) {
...
func newOrderedHandler(topology appsv1.ClusterTopology, orders []string, op int) (clusterConditionalHandler, error) {
        switch op {
