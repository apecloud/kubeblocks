...
func (c *notExistPrecondition) match(transCtx *clusterTransformContext, dag *graph.DAG, name string) (bool, error) {
	preds1, err := predecessors(c.topology, c.orders, name)
	if err != nil {
		return false, err
	}
	for _, predecessor := range preds1 {
...
func (c *phasePrecondition) match(transCtx *clusterTransformContext, dag *graph.DAG, name string) (bool, error) {
	preds2, err := predecessors(c.topology, c.orders, name)
	if err != nil {
		return false, err
	}
	for _, predecessor := range preds2 {
