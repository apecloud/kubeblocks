...
func reconcileClusterComponent(reqCtx intctrlutil.RequestCtx,
	cli client.Client,
	...
	compVers *appsv1alpha1.ClusterComponentVersion) (bool, error) {
	task := intctrltypes.ReconcileTask{
		ClusterDefinition: cd,
		ClusterVersion:    cv,
		Cluster:           cluster,
		Component:         component.BuildComponent(reqCtx, *cluster, *cd, compDef, compSpec, compVers),
		Resources:         new([]client.Object),
	}

	// build info that needs to be restored from backup
	backupSourceName, err := getClusterComponentBackupSource(cluster, task.Component.Name)
	if err != nil {
		return false, err
	}
	if len(backupSourceName) > 0 {
		if err := component.BuildRestoredInfo(reqCtx, cli, cluster.Namespace, task.Component, backupSourceName); err != nil {
			return false, err
		}
	}

	if err := buildComponentResources(reqCtx, cli, &task); err != nil {
		return false, err
	}

	// TODO: when deleting a component of the cluster, clean up the corresponding k8s resources.
	return reconcileComponentWorkloads(reqCtx, cli, cluster, cd, compSpec, *task.Resources)
}

func buildComponentResources(reqCtx intctrlutil.RequestCtx, cli client.Client, task *intctrltypes.ReconcileTask) error {
	builders := map[appsv1alpha1.WorkloadType]func(intctrlutil.RequestCtx, client.Client, *intctrltypes.ReconcileTask) error{
		appsv1alpha1.Stateless:   plan.PrepareStatelessComponentWorkloads,
		appsv1alpha1.Stateful:    plan.PrepareStatefulComponentWorkloads,
		appsv1alpha1.Consensus:   plan.PrepareConsensusComponentWorkloads,
		appsv1alpha1.Replication: plan.PrepareReplicationComponentWorkloads,
	}
	comp := task.Component
	if builder, ok := builders[comp.WorkloadType]; ok {
		return builder(reqCtx, cli, task)
	}
	return fmt.Errorf("unknown workload type: %s, namespace: %s, cluster: %s, component:%s",
		comp.WorkloadType, task.Cluster.Namespace, task.Cluster.Name, comp.Name)
}

func reconcileComponentWorkloads(reqCtx intctrlutil.RequestCtx,
	cli client.Client,
	cluster *appsv1alpha1.Cluster,
	...
	handlersByKind := map[string]func(intctrlutil.RequestCtx, client.Client, *appsv1alpha1.Cluster,
		*appsv1alpha1.ClusterDefinition, client.Object) (bool, error){
		constant.StatefulSetKind:           handleUpdateSts,
		constant.DeploymentKind:            handleUpdateDeploy,
		constant.ServiceKind:               handleUpdateSvc,
		constant.PersistentVolumeClaimKind: handleUpdatePVC,
		constant.ConfigMapKind:             handleUpdateConfigMap,
		constant.SecretKind:                handleUpdateImmutableObj, // secret objects should only be applied once.
	}
	...
	if err := plan.CreateOrCheckTLSCert(reqCtx, cli, cluster, compSpec, scheme, dbClusterFinalizerName); err != nil {
		return false, err
	}
	...
	for _, obj := range objs {
		...
		if handler, ok := handlersByKind[kind]; ok {
			requeue, err := handler(reqCtx, cli, cluster, clusterDef, obj)
			if err != nil {
				return false, err
			}
			if requeue {
	...
	if err := cleanUselessServices(reqCtx, cli, cluster, svcList); err != nil {
		return false, err
	}
...
func handleUpdateImmutableObj(_ intctrlutil.RequestCtx, _ client.Client, _ *appsv1alpha1.Cluster,
	_ *appsv1alpha1.ClusterDefinition, _ client.Object) (bool, error) {
	return false, nil
}

func handleUpdateSts(reqCtx intctrlutil.RequestCtx, cli client.Client, cluster *appsv1alpha1.Cluster,
	clusterDef *appsv1alpha1.ClusterDefinition, obj client.Object) (shouldRequeue bool, err error) {
	stsProto, ok := obj.(*appsv1.StatefulSet)
	if !ok {
		panic(fmt.Errorf("runtime error, expect a Deploy object, kind: %s, namespace: %s, name: %s",
...
func handleUpdateDeploy(reqCtx intctrlutil.RequestCtx, cli client.Client, cluster *appsv1alpha1.Cluster,
	_ *appsv1alpha1.ClusterDefinition, obj client.Object) (bool, error) {
	...
	if err := cli.Get(reqCtx.Ctx, key, deployObj); err != nil {
		return false, err
	}
	...
	if err := cli.Update(reqCtx.Ctx, deployObj); err != nil {
		return false, err
	}
...
func handleUpdateSvc(reqCtx intctrlutil.RequestCtx, cli client.Client, _ *appsv1alpha1.Cluster,
	_ *appsv1alpha1.ClusterDefinition, obj client.Object) (bool, error) {
	...
	if err := cli.Get(reqCtx.Ctx, key, svcObj); err != nil {
		return false, err
	}
	svcObj.Spec = svcProto.Spec
	svcObj.Annotations = mergeServiceAnnotations(svcObj.Annotations, svcProto.Annotations)
	return false, cli.Update(reqCtx.Ctx, svcObj)
}

// do volume expansion when workload type is `replication`
func handleUpdatePVC(reqCtx intctrlutil.RequestCtx, cli client.Client, _ *appsv1alpha1.Cluster,
	_ *appsv1alpha1.ClusterDefinition, obj client.Object) (bool, error) {
	...
	if err := cli.Get(reqCtx.Ctx, key, pvcObj); err != nil {
		return false, err
	}
	...
	return false, cli.Patch(reqCtx.Ctx, pvcObj, patch)
}
...
func handleUpdateConfigMap(reqCtx intctrlutil.RequestCtx, cli client.Client, cluster *appsv1alpha1.Cluster,
	clusterDef *appsv1alpha1.ClusterDefinition, obj client.Object) (bool, error) {
	...
	case len(cm.Labels[constant.AppConfigTypeLabelKey]) > 0:
		...
		if err := cli.Update(reqCtx.Ctx, cm); err != nil {
			return false, err
		}
	case len(cm.Labels[constant.CMConfigurationProviderTplLabelKey]) > 0:
		...
		if err := cli.Get(reqCtx.Ctx, client.ObjectKeyFromObject(cm), oldCm); err != nil {
			return false, err
		}
		...
		if clusterDefComp == nil {
			return false, errors.New("clusterDefComp not found")
		}
...
func cleanUselessServices(reqCtx intctrlutil.RequestCtx, cli client.Client, cluster *appsv1alpha1.Cluster,
	expSvcList []*corev1.Service) error {
	var (
...
// getClusterComponentBackupSource gets the backup source name for a cluster component
func getClusterComponentBackupSource(cluster *appsv1alpha1.Cluster, compName string) (string, error) {
	sources, err := getClusterBackupSourceMap(cluster)
	if err != nil {
		return "", err
	}
	return sources[compName], nil
}
