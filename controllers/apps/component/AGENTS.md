...
- `transformer_component_*.go`: transformer implementations for deletion, metadata, validation, services, accounts, TLS, vars, templates, RBAC, workload, lifecycle hooks, status, and notifications.
...
- Transformer order matters — `component_controller.go` defines the chain and `graph.ErrPrematureStop` short-circuits the remaining chain. Document ordering assumptions in comments.
