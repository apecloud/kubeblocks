...
var _ = Describe("component service transformer test", func() {
	...
	Context("pod service", func() {
		...
		DescribeTable("queries Pods in the data placement and retains their Service assistant objects", func(source string) {
			transCtx.SynthesizeComponent.Replicas = 1
			transCtx.RunningWorkload.Spec.Replicas = ptr.To[int32](1)
			transCtx.SynthesizeComponent.EnableInstanceAPI = ptr.To(true)
			transCtx.SynthesizeComponent.Annotations = map[string]string{constant.KBAppMultiClusterPlacementKey: "test-context"}
			switch source {
			case "component":
				transCtx.Component.Annotations = transCtx.SynthesizeComponent.Annotations
				transCtx.RunningWorkload.Annotations = map[string]string{constant.KBAppMultiClusterPlacementKey: "old-context"}
			case "running":
				transCtx.RunningWorkload.Annotations = transCtx.SynthesizeComponent.Annotations
				transCtx.SynthesizeComponent.AnnotationsInjectedToWorkload = map[string]string{constant.KBAppMultiClusterPlacementKey: "other-context"}
			case "desired":
				transCtx.RunningWorkload = nil
				transCtx.SynthesizeComponent.AnnotationsInjectedToWorkload = transCtx.SynthesizeComponent.Annotations
			case "context":
				transCtx.Context = multicluster.IntoContext(ctx, "test-context")
			}
			listedPods := false
			...
			Entry("Component placement takes precedence", "component"),
			Entry("falls back to the running InstanceSet", "running"),
			Entry("uses desired placement before the InstanceSet exists", "desired"),
			Entry("preserves an existing context without placement annotations", "context"),
		)
