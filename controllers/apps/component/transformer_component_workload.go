...
func copyAndMergeITS(oldITS, newITS *workloads.InstanceSet) *workloads.InstanceSet {
	...
	preserveLegacyConfigManagerPodSpec(oldITS, itsObjCopy)
...
func preserveLegacyConfigManagerPodSpec(oldITS, newITS *workloads.InstanceSet) {
	if oldITS == nil || newITS == nil {
		return
	}
	oldSpec := &oldITS.Spec.Template.Spec
	newSpec := &newITS.Spec.Template.Spec
	_, oldCfg := intctrlutil.GetContainerByName(oldSpec.Containers, legacyConfigManagerContainerName)
	if oldCfg == nil {
		return
	}
	// If another template change is already going to recreate Pods, we can piggyback on that restart
	// to drop the legacy config-manager resources instead of carrying them forward forever.
	if shouldCleanupLegacyConfigManager(oldITS, newITS) {
		return
	}
...
func shouldCleanupLegacyConfigManager(oldITS, newITS *workloads.InstanceSet) bool {
	oldTemplate := stripLegacyConfigManagerPodTemplate(oldITS.Spec.Template)
	newTemplate := stripLegacyConfigManagerPodTemplate(newITS.Spec.Template)
	...
	// Clean up only when the configured policy will recreate Pods, because PreferInPlace upgrades should
	// keep the live template aligned with old Pods that still carry the legacy sidecar.
	if hasPodUpgradeTemplateChanges(oldTemplate, newTemplate) {
		return newITS.Spec.PodUpgradePolicy == appsv1.ReCreatePodUpdatePolicyType
	}
	...
	if hasPodUpdateTemplateChanges(oldTemplate, newTemplate) {
		return newITS.Spec.PodUpdatePolicy == appsv1.ReCreatePodUpdatePolicyType
	}
	return false
}
