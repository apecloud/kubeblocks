...
func (c *StatefulComponentBase) Status(reqCtx intctrlutil.RequestCtx, cli client.Client, builder ComponentWorkloadBuilder) error {
	...
	if err := c.updateTemplateRenderedConfig(); err != nil {
		return err
	}
...
// updateTemplateRenderedConfig transform the action of rendered config by config-template from noop to update in cluster Status reconciliation
func (c *StatefulComponentBase) updateTemplateRenderedConfig() error {
	for _, v := range ictrltypes.FindAll[*corev1.ConfigMap](c.Dag) {
		...
		_, ok := cm.GetLabels()[constant.CMConfigTemplateNameLabelKeyInCue]
