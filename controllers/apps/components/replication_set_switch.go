...
func (s *Switch) updateRoleLabel() error {
	...
	if err := GetObjectListByComponentName(s.SwitchResource.Ctx, s.SwitchResource.Cli,
		*s.SwitchResource.Cluster, stsList, s.SwitchResource.CompSpec.Name); err != nil {
		return err
	}

	for _, sts := range stsList.Items {
		if IsMemberOf(&sts, s.SwitchInstance.OldPrimaryRole.Pod) {
		...
		if IsMemberOf(&sts, s.SwitchInstance.CandidatePrimaryRole.Pod) {
