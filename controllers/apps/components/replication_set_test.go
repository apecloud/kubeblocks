...
var _ = Describe("Replication Component", func() {
	...
	Context("Replication Component test", func() {
		It("Replication Component test", func() {
			...
			replicationSetSts := apps.NewStatefulSetFactory(testCtx.DefaultNamespace,
			...
			compDefName := clusterObj.Spec.GetComponentDefRefName(apps.DefaultRedisCompSpecName)
			componentDef := clusterDefObj.GetComponentDefByName(compDefName)
			componentSpec := clusterObj.Spec.GetComponentByName(apps.DefaultRedisCompSpecName)
			...
			for _, availableReplica := range []int32{0, replicas} {
				...
				if availableReplica > 0 {
					...
					stsPods := apps.MockReplicationComponentPods(nil, testCtx, replicationSetSts, clusterObj.Name,
						apps.DefaultRedisCompSpecName, map[int32]string{
							0: constant.Primary,
			...
			testk8s.PatchStatefulSetStatus(&testCtx, replicationSetSts.Name, status)
			phase, _, _ := replicationComponent.GetPhaseWhenPodsNotReady(ctx, apps.DefaultRedisCompSpecName, false)
			...
			phase, _, _ = replicationComponent.GetPhaseWhenPodsNotReady(ctx, apps.DefaultRedisCompSpecName, true)
			Expect(phase).Should(Equal(appsv1alpha1.AbnormalClusterCompPhase))

			// mock pod label is empty
			Expect(apps.ChangeObj(&testCtx, primaryPod, func(pod *corev1.Pod) {
				pod.Labels[constant.RoleLabelKey] = ""
			})).Should(Succeed())
			_, statusMessages, _ := replicationComponent.GetPhaseWhenPodsNotReady(ctx, apps.DefaultRedisCompSpecName, false)
			...
			Expect(apps.ChangeObj(&testCtx, clusterDefObj, func(clusterDef *appsv1alpha1.ClusterDefinition) {
			...
			_ = apps.MockReplicationComponentPod(nil, testCtx, replicationSetSts, clusterObj.Name, apps.DefaultRedisCompSpecName, newPodName, "")
			vertexes, err = replicationComponent.HandleRoleChange(ctx, replicationSetSts)
			Expect(err).To(Succeed())
