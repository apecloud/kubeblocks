...
func (actionCtx ActionContext) createActionK8sWorkload(
	...
	targetPodName string) (*appsv1alpha1.ActionTask, error) {
	...
	return &appsv1alpha1.ActionTask{
		Namespace:     workload.GetNamespace(),
		ObjectKey:     objectKey,
		TargetPodName: targetPodName,
		Status:        appsv1alpha1.ProcessingActionTaskStatus,
	}, nil
}

func (actionCtx ActionContext) checkActionStatus(progressDetail appsv1alpha1.ProgressStatusDetail,
	checkTaskStatus func(ActionContext, *appsv1alpha1.ActionTask, int) (bool, bool, error)) (*ActionStatus, error) {
	var (
	...
	for i := range tasks {
		...
		if failed {
			existFailed = true
			tasks[i].Status = appsv1alpha1.FailedActionTaskStatus
		} else {
			tasks[i].Status = appsv1alpha1.SucceedActionTaskStatus
		}
	}
	return &ActionStatus{
		ActionTasks:  tasks,
