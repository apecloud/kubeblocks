	...
	Context("Test OpsRequest", func() {
		...
		testAbortHorizontalScaling := func(isScalingOut bool) {
			checkProgressStatus := func(opsRes *OpsResource, expectCompReplicas int, expectPodStatus map[string]appsv1alpha1.ProgressStatus) {
				Expect(opsRes.Cluster.Spec.GetComponentByName(defaultCompName).Replicas).Should(BeEquivalentTo(expectCompReplicas))
			...
			Expect(opsRes.Cluster.Spec.GetComponentByName(defaultCompName).Replicas).Should(BeEquivalentTo(targetReplicas))

			By("mock progress detail")
			pod1Name := pods[2].Name
			pod2Name := pods[1].Name
			if isScalingOut {
				pod1Name = fmt.Sprintf("%s-%s-3", clusterName, defaultCompName)
				pod2Name = fmt.Sprintf("%s-%s-4", clusterName, defaultCompName)
				createPods("", 3)
			} else {
				deletePods(pods[2])
			}
			...
			Expect(HandleAbortForHScale(reqCtx, k8sClient, opsRes)).ShouldNot(HaveOccurred())
			Expect(opsRes.Cluster.Spec.GetComponentByName(defaultCompName).Replicas).Should(BeEquivalentTo(abortReplicas))
