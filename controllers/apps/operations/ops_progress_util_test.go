...
var _ = Describe("Ops ProgressDetails", func() {
	...
	testProgressDetailsWithStatefulPodUpdating := func(reqCtx intctrlutil.RequestCtx, opsRes *OpsResource, pods []*corev1.Pod) {
		...
		Expect(getProgressDetailStatus(opsRes, defaultCompName, pod)).Should(Equal(appsv1alpha1.ProcessingProgressStatus))
		...
		_, _ = GetOpsManager().Reconcile(reqCtx, k8sClient, opsRes)
		Expect(getProgressDetailStatus(opsRes, defaultCompName, pod)).Should(Equal(appsv1alpha1.SucceedProgressStatus))
	...
	Context("Test Ops ProgressDetails", func() {
		...
		It("Test Ops ProgressDetails with scale-in replicas", func() {
			...
			testapps.MockInstanceSetStatus(testCtx, opsRes.Cluster, defaultCompName)
			_, _ = GetOpsManager().Reconcile(reqCtx, k8sClient, opsRes)
			Expect(getProgressDetailStatus(opsRes, defaultCompName, targetPod)).Should(Equal(appsv1alpha1.SucceedProgressStatus))
