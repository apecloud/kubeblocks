...
func (r rebuildInstanceOpsHandler) rebuildInstanceWithNoBackup(reqCtx intctrlutil.RequestCtx,
	...
	return r.instanceIsAvailable(reqCtx, cli, opsRes, insHelper.synthesizedComp, insHelper.targetPod)
}
	...
	waitRestoreCompleted := func(stage dpv1alpha1.RestoreStage) (bool, error) {
		...
		if restore == nil {
			...
			if stage == dpv1alpha1.PostReady {
				...
				available, err := r.instanceIsAvailable(reqCtx, cli, opsRes, insHelper.synthesizedComp, insHelper.targetPod)
...
func (r rebuildInstanceOpsHandler) instanceIsAvailable(
	...
	if !podutils.IsPodAvailable(targetPod, minReadySeconds, metav1.Now()) {
		return false, nil
	}
