...
const (
	...
	sourcePVReclaimPolicyAnnotation = "apps.kubeblocks.io/source-reclaim-policy"
...
func (inPlaceHelper *inplaceRebuildHelper) buildRestoreMetaObject(opsRequest *appsv1alpha1.OpsRequest, restoreName string) metav1.ObjectMeta {
...
func (inPlaceHelper *inplaceRebuildHelper) rebuildSourcePVCsAndRecreateInstance(reqCtx intctrlutil.RequestCtx,
	cli client.Client,
	opsRequest *appsv1alpha1.OpsRequest,
	progressDetail *appsv1alpha1.ProgressStatusDetail) error {
	for sourcePVCName, v := range inPlaceHelper.pvcMap {
		...
		if err = cli.Get(reqCtx.Ctx, types.NamespacedName{Name: sourcePVCName, Namespace: tmpPVC.Namespace}, sourcePvc); err != nil {
			// if not exists, wait for the pvc to be recreated by the external controller.
			return err
		}
