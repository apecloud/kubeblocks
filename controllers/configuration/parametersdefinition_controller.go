...
func (r *ParametersDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	parametersDef := &configurationv1alpha1.ParametersDefinition{}
...
func (r *ParametersDefinitionReconciler) reconcile(reqCtx intctrlutil.RequestCtx, parametersDef *configurationv1alpha1.ParametersDefinition) (ctrl.Result, error) {
	...
	if err := updateParamDefinitionStatus(r.Client, reqCtx, parametersDef, configurationv1alpha1.PDAvailablePhase); err != nil {
		return intctrlutil.CheckedRequeueWithError(err, reqCtx.Log, "")
	}
...
func (r *ParametersDefinitionReconciler) deletionHandler(parametersDef *configurationv1alpha1.ParametersDefinition, reqCtx intctrlutil.RequestCtx) func() (*ctrl.Result, error) {
	recordEvent := func() {
	...
	return func() (*ctrl.Result, error) {
		if parametersDef.Status.Phase != configurationv1alpha1.PDDeletingPhase {
			err := updateParamDefinitionStatus(r.Client, reqCtx, parametersDef, configurationv1alpha1.PDDeletingPhase)
			if err != nil {
				return nil, err
			}
		}
...
func updateParamDefinitionStatus(cli client.Client, ctx intctrlutil.RequestCtx, parametersDef *configurationv1alpha1.ParametersDefinition, phase configurationv1alpha1.ParametersDescPhase) error {
...
func checkParametersSchema(ctx intctrlutil.RequestCtx, parametersDef *configurationv1alpha1.ParametersDefinition) (bool, error) {
	// validate configuration template
	validateConfigSchema := func(ccSchema *configurationv1alpha1.ParametersSchema) (bool, error) {
	...
	if ok, err := validateConfigSchema(parametersDef.Spec.ParametersSchema); !ok || err != nil {
		ctx.Log.Error(err, "failed to validate template schema!", "configMapName", fmt.Sprintf("%v", parametersDef.Spec.ParametersSchema))
		return ok, err
	}
	return true, nil
}

func updateParametersSchema(parametersDef *configurationv1alpha1.ParametersDefinition, cli client.Client, ctx context.Context) error {
