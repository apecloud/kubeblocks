...
var _ = Describe("ConfigConstraint Controller", func() {
	...
	Context("Create config constraint with cue validate", func() {
		It("Should ready", func() {
			...
			parametersDef := testapps.NewParametersDefinitionFactory("mysql-parameters-8.0").
...
`).
			...
			Consistently(testapps.CheckObjExists(&testCtx, parametersDefKey, &configurationv1alpha1.ParametersDefinition{}, true)).Should(Succeed())
			...
			Eventually(testapps.CheckObjExists(&testCtx, parametersDefKey, &configurationv1alpha1.ParametersDefinition{}, false), time.Second*60, time.Second*1).Should(Succeed())
		})
	})
