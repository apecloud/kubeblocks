...
func (r *RestoreReconciler) inProgressAction(reqCtx intctrlutil.RequestCtx, restore *dpv1alpha1.Restore) (ctrl.Result, error) {
	restoreMgr := dprestore.NewRestoreManager(restore, r.Scheme)
	...
	if intctrlutil.IsTargetError(err, intctrlutil.ErrorTypeFatal) {
		...
		r.Recorder.Event(restore, corev1.EventTypeWarning, string(dpv1alpha1.RestorePhaseFailed), err.Error())
		err = nil
	}
