...
var _ = Describe("Volume Populator Controller test", func() {
	...
	When("volume populator controller test", func() {
		...
		testVolumePopulate := func(volumeBinding storagev1.VolumeBindingMode, useVolumeSnapshotBackup bool) {
			...
			By("keep the target PVC unbound until prepareData completes")
			Consistently(testapps.CheckObj(&testCtx, pvcKey, func(g Gomega, targetPVC *corev1.PersistentVolumeClaim) {
				g.Expect(targetPVC.Spec.VolumeName).Should(BeEmpty())
