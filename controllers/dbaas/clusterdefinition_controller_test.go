	...
	Context("When updating clusterDefinition", func() {
		It("Should update status of clusterVersion at the same time", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(clusterDefinition), cd)).To(Succeed())
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(len(cv.Finalizers) > 0 &&
					...
					cv.Status.ClusterDefGeneration == clusterDefinition.GetObjectGeneration()).To(BeTrue())
			}, timeout, interval).Should(Succeed())
		...
		It("Should stop proceeding the status of clusterDefinition", func() {
			...
			Consistently(func(g Gomega) {
				...
				g.Eventually(func() error {
					return k8sClient.Get(ctx, client.ObjectKeyFromObject(clusterDefinition), cd)
				}, timeout, interval).Should(Succeed())
