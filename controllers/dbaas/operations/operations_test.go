...
spec:
  ...
  type: %s`, opsDefinitionName, clusterDefName, opsType)
...
`, clusterName)
	...
	Context("Test OpsRequest and OpsDefinition", func() {
		It("Should Test all OpsRequest", func() {
			...
			By("Test Upgrade Ops")
			_ = createOpsDefinition("upgrade_definition", clusterDef.Name, dbaasv1alpha1.UpgradeType)
			...
			ops.Spec.ComponentOps = &dbaasv1alpha1.ComponentOps{
			...
			_, _ = GetOpsManager().MainEnter(opsRes)
			opsRes.Cluster.Status.Phase = dbaasv1alpha1.RunningPhase
			opsRes.OpsRequest.Status.Phase = dbaasv1alpha1.RunningPhase
			_, _ = GetOpsManager().MainEnter(opsRes)
			_ = GetOpsManager().ReconcileMainEnter(opsRes)
			...
			_ = PatchOpsDefinitionNotFound(opsRes)
