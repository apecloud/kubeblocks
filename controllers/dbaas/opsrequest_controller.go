...
func (r *OpsRequestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if res, err = r.patchOpsRequestWithClusterLabel(reqCtx, opsRequest); res != nil {
		return *res, err
	}

	// process opsRequest entry function
	if res, err = r.processOpsRequest(reqCtx, opsRes); res != nil {
		return *res, err
	}
...
// processOpsRequest validate OpsDefinition support the Operation and enter the opsManger MainEnter function to process the OpsRequest
func (r *OpsRequestReconciler) processOpsRequest(reqCtx intctrlutil.RequestCtx, opsRes *operations.OpsResource) (*ctrl.Result, error) {
	var (
		err    error
		res    *ctrl.Result
		opsDef *dbaasv1alpha1.OpsDefinition
	)
	if opsDef, err = r.validateDefinition(opsRes); opsDef == nil {
		return nil, nil
	}
	if err == nil {
		res, err = operations.GetOpsManager().MainEnter(opsRes)
	}
	if err != nil {
...
// validateOpsDefinition validate whether the cluster of OpsRequest.spec.clusterRef support the operation
func (r *OpsRequestReconciler) validateDefinition(opsRes *operations.OpsResource) (*dbaasv1alpha1.OpsDefinition, error) {
	var (
		opsDefinitionList = &dbaasv1alpha1.OpsDefinitionList{}
		opsDef            *dbaasv1alpha1.OpsDefinition
	)
	if err := opsRes.Client.List(opsRes.Ctx, opsDefinitionList, client.MatchingLabels{clusterDefLabelKey: opsRes.Cluster.Spec.ClusterDefRef}); err != nil {
		return nil, err
	}

	for _, v := range opsDefinitionList.Items {
		if v.Spec.Type == opsRes.OpsRequest.Spec.Type {
			opsDef = &v
			break
		}
	}
	if opsDef == nil {
		return nil, operations.PatchOpsDefinitionNotFound(opsRes)
	}
	if opsDef.Status.Phase == dbaasv1alpha1.UnavailablePhase {
		return nil, operations.PatchOpsDefinitionUnavailable(opsRes, opsDef.Name)
	}
	return opsDef, nil
}
...
func (r *OpsRequestReconciler) patchOpsRequestWithClusterLabel(reqCtx intctrlutil.RequestCtx, opsRequest *dbaasv1alpha1.OpsRequest) (*ctrl.Result, error) {
	...
	return &ctrl.Result{}, r.Client.Patch(reqCtx.Ctx, opsRequest, patch)
}
