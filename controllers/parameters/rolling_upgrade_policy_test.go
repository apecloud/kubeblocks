...
var _ = Describe("Reconfigure RollingPolicy", func() {
	...
	Context("consensus rolling reconfigure policy test", func() {
		It("Should success without error", func() {
			...
			Expect(err).Should(Succeed())
			Expect(status.Status).Should(BeEquivalentTo(ESRetry))
			Expect(mockPods[acc][2].Labels[mockParam.generateConfigIdentifier()]).Should(BeEquivalentTo(mockParam.getTargetVersionHash()))
			Expect(mockPods[acc][1].Labels[mockParam.generateConfigIdentifier()]).ShouldNot(BeEquivalentTo(mockParam.getTargetVersionHash()))
			Expect(mockPods[acc][0].Labels[mockParam.generateConfigIdentifier()]).ShouldNot(BeEquivalentTo(mockParam.getTargetVersionHash()))

			// upgrade pod-2
			status, err = rollingPolicy.Upgrade(mockParam)
			Expect(err).Should(Succeed())
			Expect(status.Status).Should(BeEquivalentTo(ESRetry))
			Expect(mockPods[acc][0].Labels[mockParam.generateConfigIdentifier()]).Should(BeEquivalentTo(mockParam.getTargetVersionHash()))
			Expect(mockPods[acc][1].Labels[mockParam.generateConfigIdentifier()]).ShouldNot(BeEquivalentTo(mockParam.getTargetVersionHash()))
			...
			Expect(err).Should(Succeed())
			Expect(status.Status).Should(BeEquivalentTo(ESRetry))
			Expect(mockPods[acc][1].Labels[mockParam.generateConfigIdentifier()]).Should(BeEquivalentTo(mockParam.getTargetVersionHash()))
