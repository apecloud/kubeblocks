...
const (
	...
	// advance the annotation to the newAnnotation value.
	...
	// and must not change the annotation.
	roleProbeGateRejectMalformed
)

// checkEngineVersionStale decides whether the parsed roleProbe event should
// advance the Pod role label, and what the new annotation value should be
// when accepted. The four legitimate annotation/mode combinations are
// handled explicitly:
//
//   - engine event vs engine annotation: numeric compare; reject unless
//     strictly newer.
...
//     cannot be silently downgraded back to EventTime by an unaware
//     emitter.
//   - legacy event vs legacy bare EventTime annotation: bare-numeric
//     EventTime compare (preserves the pre-existing behaviour for addons
//     that have not adopted the new contract).
//
// A Malformed event is always rejected and leaves the annotation unchanged.
func checkEngineVersionStale(parsed roleProbeOutput, lastAnnotation string, eventTimeMicros int64) (roleProbeGateDecision, string) {
	switch parsed.mode {
