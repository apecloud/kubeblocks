...
if which mongosh; then
  ...
  until mongo --quiet --port $PORT --eval "print('I am ready')"; do sleep 1; done

  until mongo --quiet --port $PORT --eval "rs.status().set" -u root --password $MONGODB_ROOT_PASSWORD || \
  mongo --quiet --port $PORT --eval "try { rs.status().set } catch (e) { if (e.codeName=='NotYetInitialized') {print('')} else {exit(1)} }" ; do
    sleep 1;
  done

  if mongo --quiet --port $PORT --eval "rs.status().set" -u root --password $MONGODB_ROOT_PASSWORD; then
    is_inited=$(mongo --quiet --port $PORT --eval "rs.status().set" -u root --password $MONGODB_ROOT_PASSWORD)
  fi

  if mongo --quiet --port $PORT --eval "try { rs.status().set } catch (e) { if (e.codeName=='NotYetInitialized') {print('')} else {exit(1)} }"; then
    is_inited=$(mongo --quiet --port $PORT --eval "try { rs.status().set } catch (e) { if (e.codeName=='NotYetInitialized') {print('')} else {exit(1)} }")
  fi

  echo is_inited: $is_inited

  if [ $is_inited ]; then
    if mongo --quiet --port $PORT --eval "rs.status().set"; then
      echo "reset password"
      (until mongo --quiet --port $PORT --eval "rs.isMaster().ismaster"|grep true; do sleep 1; done;
      mongo --quiet --port $PORT admin --eval "db.createUser({ user: '$MONGODB_ROOT_USER', pwd: '$MONGODB_ROOT_PASSWORD', roles: [{role: 'root', db: 'admin'}] })") </dev/null  >/dev/null 2>&1 &
    fi
    exit 0
  fi
  ...
  while [ $i -lt $(eval echo \$KB_"$TYPE_NAME"_N) ]; do
    host=$(eval echo \$KB_"$TYPE_NAME"_"$i"_HOSTNAME)
    host=$host"."$KB_NAMESPACE".svc.cluster.local"
    until mongo --quiet --port $PORT --host $host --eval "print('peer is ready')"; do sleep 1; done
    if [ $i -eq 0 ]; then
      MEMBERS="{_id: $i, host: \"$host:$PORT\", priority:2}"
    else
      MEMBERS="$MEMBERS,{_id: $i, host: \"$host:$PORT\"}"
  ...
  sleep 10
  set -e
  mongo --quiet --port $PORT --eval "rs.initiate({_id: \"$RPL_SET_NAME\", $CONFIGSVR members: [$MEMBERS]})";
  set +e
