...
exec /usr/local/nebula/bin/nebula-metad --flagfile=/usr/local/nebula/etc/nebula-metad.conf --meta_server_addrs={{ $metads }} --local_ip=$KB_POD_FQDN".cluster.local" --ws_ip=$KB_POD_FQDN".cluster.local" --daemonize=false