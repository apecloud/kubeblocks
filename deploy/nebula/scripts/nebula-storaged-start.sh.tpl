...
{{- end }}
{{- $metads = trimPrefix "," $metads }}
exec /usr/local/nebula/bin/nebula-storaged --flagfile=/usr/local/nebula/etc/nebula-storaged.conf --meta_server_addrs={{ $metads }} --local_ip=$KB_POD_FQDN".cluster.local" --ws_ip=$KB_POD_FQDN".cluster.local" --daemonize=false