...
echo "user $REDIS_REPL_USER on +psync +replconf +ping >$REDIS_PASSWORD" >> /data/users.acl
echo "user $REDIS_SENTINEL_USER on allchannels +multi +slaveof +ping +exec +subscribe +config|rewrite +role +publish +info +client|setname +client|kill +script|kill >$REDIS_PASSWORD" >> /data/users.acl
