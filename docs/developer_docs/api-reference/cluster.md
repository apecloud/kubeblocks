...
<p>When a pod is about to be updated, a switchover action will be triggered to it. So addon implementation must determine
if the pod&rsquo;s current role needs to be transferred. Due to this reason, targetPodSelector is not applicable for switchover action.</p>
<p>The container executing this action has access to following variables:</p>
<ul>
