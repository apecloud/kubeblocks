...
- For KubeBlocks v0.6, newly created Redis clusters need password to connect, which is not supported by v0.5.
- For KubeBlocks v0.6, the backup and restore function is updated greatly. Manually update the 0.5 version cluster to 0.6, otherwise you can not use the new function.
- For KubeBlocks v0.6, password-free login function is canceled for Postgres cluster, and password is required to access the newly created clusters. When restoring, if the cluster is in creating status for a long time, you can check logs to see whether there is `password authentication failed error`, you can update password to fix this.
...
- Specifiy a configuration file for initialization settings when installing KubeBlocks
