...
For Feishu, obtain the webhook address after creating a custom robot. If the signature verification in the security configuration has been enabled, obtain the signature key in advance.

Currently, Feishu custom bot, DingTalk custom bot, WeChat Enterprise custom bot, and Slack are supported. Refer to the following guides to configure notification channels.
...
* Each notification channel has its interface calls and frequency limits and when the limits are reached, rate limiting is applied and you cannot receive alerts.
