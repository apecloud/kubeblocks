...
function download_k3s_images() {
   ...
   if [ -f "${K3S_IMAGE_DIR}/${K3S_IMAGE_NAME}" ]; then
   	...
   	curl -Lo ${K3S_IMAGE_NAME}/${K3S_IMAGE_NAME} https://github.com/k3s-io/k3s/releases/download/${K3S_VERSION}/k3s-airgap-images-${ARCH}.tar.gz
   fi
}

function download_k3s_bin_script() {
  	mkdir -p ${K3S_OTHER_DIR}
  	curl -Lo ${K3S_OTHER_DIR}/k3s https://github.com/k3s-io/k3s/releases/download/${K3S_VERSION}/k3s
