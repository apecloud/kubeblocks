...
# https://jihulab.com/api/v4/projects/85948/packages/generic/kubeblocks/v0.5.0-test-win/kbcli-linux-amd64.tar.gz
downloadFile() {
...
installFile() {
    ...
    local tmp_root_kbcli="$CLI_TMP_ROOT/${OS}-${ARCH}-${LATEST_RELEASE_TAG}/$CLI_FILENAME"
...
downloadFile() {
    ...
    echo "Downloading ..."
    if [ "$HTTP_REQUEST_CLI" == "curl" ]; then
        curl -SLf --header 'Accept:application/octet-stream' "$DOWNLOAD_URL" -o "$ARTIFACT_TMP_FILE"
    else
        wget -q --show-progress -O "$ARTIFACT_TMP_FILE" "$DOWNLOAD_URL"
    fi
