...
ERR=false
FAIL=false
EXCLUDES_DIRS="vendor/\|apis/\|tools/\|externalapis/\|pkg/cli/cmd/plugin/download"
APACHE2_DIRS="apis/\|externalapis/"

HEADER="Copyright (C) 2022-2023 ApeCloud Co., Ltd\|Copyright (C) 2022-2024 ApeCloud Co., Ltd\|Code generated by"
for file in $(git ls-files | grep '\.cue\|\.go$' | grep -v ${EXCLUDES_DIRS}); do
  echo -n "Header check: $file... "
  if [[ -z $(cat ${file} | grep "${HEADER}") ]]; then
      ERR=true
  fi
  if [ $ERR == true ]; then
