...
keywords: [starrocks, analytic, data warehouse, control plane, 数据仓库, 分析型数据库]
sidebar_position: 1
...
本教程演示了如何通过 `kubectl` 或 YAML 文件创建并管理 StarRocks 集群。您可在 [GitHub 仓库](https://github.com/apecloud/kubeblocks-addons/tree/release-0.9/examples/starrocks)查看相应的 YAML 示例和指南。
...
* 查看可用于创建集群的数据库类型和版本。
  ...
  确保 `starrocks` cluster definition 已安装。如果该 cluster definition 不可用，可参考[相关文档](./../installation/install-addons.md)启用。
