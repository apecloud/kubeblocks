...
本教程演示了如何通过 `kubectl` 或 YAML 文件创建并管理 Weaviate 集群。您可在 [GitHub 仓库](https://github.com/apecloud/kubeblocks-addons/tree/release-0.9/examples/weaviate)查看相应的 YAML 示例和指南。
...
* 查看可用于创建集群的数据库类型和版本。
  ...
  确保 `weaviate` cluster definition 已安装。如果该 cluster definition 不可用，可[参考相关文档](./../installation/install-addons.md)启用。
...
KubeBlocks 通过 `Cluster` 定义集群。以下是创建 Weaviate 集群的示例。Pod 默认分布在不同节点。但如果您只有一个节点可用于部署集群，可将 `spec.affinity.topologyKeys` 设置为 `null`。
...
| `spec.componentSpecs.componentDefRef` | 表示 cluster definition 中定义的 component definition 的名称，可通过执行 `kubectl get clusterdefinition weaviate -o json \| jq '.spec.componentDefs[].name'` 命令获取 component definition 名称。 |
...
Weaviate 通过 8080 端口提供 HTTP 协议供客户端访问，您可以通过本地主机访问集群。
