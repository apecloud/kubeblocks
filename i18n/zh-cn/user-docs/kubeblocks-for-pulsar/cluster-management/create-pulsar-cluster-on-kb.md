...
KubeBlocks 可以通过良好的抽象快速集成新引擎，并支持 Pulsar 集群的创建和删除、集群组件的垂直扩容和水平扩容、卷扩展、重启和配置更改等。

本系列文档重点展示 KubeBlocks 对 Pulsar 日常运维能力的支持，包括集群创建、删除、重启等基本生命周期操作，以及水平扩容、垂直扩容、卷扩展、配置变更、监控等高阶操作。
...
1. 在本地创建 `helm` 使用的 Pulsar 集群模板文件 `values-production.yaml`。
   ...
   将以下信息复制到本地文件 `values-production.yamlCre` 中。
