...
func NewHeadlessServiceBuilder(namespace, name string) *ServiceBuilder {
	...
	builder.GetObject().Spec.ClusterIP = corev1.ClusterIPNone
	return builder
}
...
func (builder *ServiceBuilder) AddSelectorsInMap(keyValues map[string]string) *ServiceBuilder {
	selectors := builder.GetObject().Spec.Selector
	if selectors == nil {
...
func (builder *ServiceBuilder) SetType(serviceType corev1.ServiceType) *ServiceBuilder {
	...
	builder.GetObject().Spec.Type = serviceType
	if serviceType == corev1.ServiceTypeLoadBalancer {
		...
		builder.GetObject().Spec.ExternalTrafficPolicy = corev1.ServiceExternalTrafficPolicyTypeLocal
	}
	return builder
}
