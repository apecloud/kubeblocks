...
var _ = Describe("sync2foxlake", func() {
	...
	It("func test", func() {
		e := EndpointModel{}
		Expect(e.complete("")).Should(HaveOccurred())

		Expect(e.complete("test")).Should(Succeed())
		Expect(e.EndpointType).Should(Equal(ClusterNameEndpointType))
		Expect(e.Endpoint).Should(Equal("test"))

		Expect(e.complete("user:123456@127.0.0.1:5432")).Should(Succeed())
		...
		Expect(e.complete("user@host")).Should(HaveOccurred())
	})
