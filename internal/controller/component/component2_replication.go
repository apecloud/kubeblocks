...
func (c *replicationComponent) init(reqCtx intctrlutil.RequestCtx, cli client.Client) error {
	...
	for i := int32(0); i < synthesizedComp.Replicas; i++ {
		builder.buildEnv(). // TODO: workload related, scaling related
					buildReplicationWorkload(i). // build workload here since other objects depend on it.
					buildReplicationVolume(i).
	...
	return builder.buildService(patchService).buildTLSCert().complete()
}
...
func (c *replicationComponent) ExpandVolume(reqCtx intctrlutil.RequestCtx, cli client.Client) error {
	...
	for _, sts := range workloads {
		for _, vct := range c.Component.VolumeClaimTemplates {
			...
			if vertex := lifecycle.FindMatchedVertex(c.Dag, key); vertex == nil {
				...
				vertex.(*lifecycleVertex).action = lifecycle.UPDATE
			}
		}
	}
	return nil
