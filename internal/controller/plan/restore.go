...
func (p *RestoreManager) getRecoveryInfo(baseBackup, logfileBackup *dpv1alpha1.Backup) (*dpv1alpha1.BackupToolSpec, error) {
	...
	if envTimeEnvIdx != -1 {
		...
		headEnv = append(headEnv, corev1.EnvVar{Name: "KB_RECOVERY_TIME", Value: p.restoreTime.UTC().Format(timeFormat)})
	}
	headEnv = append(headEnv, corev1.EnvVar{Name: "KB_RECOVERY_TIMESTAMP", Value: strconv.FormatInt(p.restoreTime.Unix(), 10)})
	...
	if baseBackup.Status.Manifests != nil {
		...
		if backupLog != nil && backupLog.StartTime != nil {
			startTimeEnv := corev1.EnvVar{Name: "BACKUP_START_TIME", Value: backupLog.StartTime.UTC().Format(timeFormat)}
			startTimeTimestampEnv := corev1.EnvVar{Name: "BACKUP_START_TIMESTAMP", Value: strconv.FormatInt(backupLog.StartTime.Unix(), 10)}
...
func (p *RestoreManager) buildLogicRestoreJob(synthesizedComponent *component.SynthesizedComponent, backup *dpv1alpha1.Backup, envs ...corev1.EnvVar) (objs []client.Object, err error) {
	...
	for pvcName, pod := range pvcsAndPodsMap {
		...
		podENV = append(podENV, corev1.EnvVar{Name: "DB_HOST", Value: intctrlutil.BuildPodHostDNS(&pod)})
