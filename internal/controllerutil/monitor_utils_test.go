...
func TestCalcCharacterType(t *testing.T) {
	{
		expectType := "mysql"
		actualType := CalcCharacterType("stat.mysql", "replicasets")
		assertEqual(t, expectType, actualType)
	}

	{
		expectType := "mysql"
		actualType := CalcCharacterType("state.mysql-8", "replicasets")
		assertEqual(t, expectType, actualType)
	}

	{
		expectType := ""
		actualType := CalcCharacterType("other", "replicasets")
		assertEqual(t, expectType, actualType)
	}
}
