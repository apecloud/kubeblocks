...
// ListCompletedBackupsByType lists the completed backup by type.
/*func ListCompletedBackupsByType(reqCtx intctrlutil.RequestCtx,
	cli client.Client,
	sourceBackup *dpv1alpha1.Backup,
	backupType dpv1alpha1.BackupType) ([]dpv1alpha1.Backup, error) {
	backups := dpv1alpha1.BackupList{}
	if err := cli.List(reqCtx.Ctx, &backups,
		client.InNamespace(sourceBackup.Namespace),
		client.MatchingLabels(map[string]string{
			dptypes.DataProtectionLabelBackupPolicyKey: sourceBackup.Spec.BackupPolicyName,
			dptypes.BackupTypeLabelKeyKey:              string(backupType),
		}),
	); err != nil {
		return nil, err
	}
	// TODO: check if backup from the same target.
	backupItems := make([]dpv1alpha1.Backup, 0, len(backups.Items))
...
func SortBackups(backups []dpv1alpha1.Backup, reverse bool) []dpv1alpha1.Backup {
	sort.Slice(backups, func(i, j int) bool {
		if reverse {
			i, j = j, i
		}
		return CompareWithBackupStopTime(backups[i], backups[j])
	})
	return backups
}*/
