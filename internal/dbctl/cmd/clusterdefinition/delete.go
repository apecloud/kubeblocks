...
var deleteExample = templates.Examples(`
		# delete a clusterdefinition named my-cluster-def
		dbctl clusterdefinition delete my-cluster-def
`)

func NewDeleteCmd(f cmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	return builder.NewCmdBuilder().
		Use("delete").
		Short("Delete a clusterdefinition").
		Example(deleteExample).
		GroupKind(types.ClusterDefGK()).
		Factory(f).
		IOStreams(streams).
		Build(delete.Build)
}
