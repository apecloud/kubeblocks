	...
	Context("AnalyzeHostUtility test", func() {
		BeforeEach(func() {
			analyzer = AnalyzeClusterRegion{
				analyzer: &preflightv1beta2.ClusterRegion{
					...
					RegionList: []string{"beijing", "shanghai"}}}
		})
		It("Analyze test, and get file failed", func() {
			Eventually(func(g Gomega) {
				...
				_, err := analyzer.Analyze(getCollectedFileContents, nil)
				g.Expect(err).Should(HaveOccurred())
			}, timeOut).Should(Succeed())
		})

		It("Analyze test, and return of get file is not corev1.NodeList", func() {
			Eventually(func(g Gomega) {
				getCollectedFileContents := func(filename string) ([]byte, error) {
