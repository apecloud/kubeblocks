...
var _ = Describe("utils", func() {
	...
	Context("validate component class", func() {
		...
		BeforeEach(func() {
			var err error
			kbClassFactory := apps.NewComponentClassDefinitionFactory(kbClassDefinitionObjName, clusterDefinitionName, compType1)
			kbClassFactory.AddClasses([]v1alpha1.ComponentClass{
			...
			customClassFactory := apps.NewComponentClassDefinitionFactory(customClassDefinitionObjName, clusterDefinitionName, compType1)
			customClassFactory.AddClasses([]v1alpha1.ComponentClass{
				buildClass("large", "100", "200Gi"),
			})

			constraint := apps.NewComponentResourceConstraintFactory(apps.DefaultResourceConstraintName).
			...
			It("should succeed with valid classDefRef", func() {
				comp := &v1alpha1.ClusterComponentSpec{
					...
					ClassDefRef: &v1alpha1.ClassDefRef{
						...
						Class: apps.Class1c1g.Name,
					},
				}
				cls, err := clsMgr.ChooseClass(comp)
				Expect(err).ShouldNot(HaveOccurred())
				Expect(cls.CPU.Equal(apps.Class1c1g.CPU)).Should(BeTrue())
				Expect(cls.Memory.Equal(apps.Class1c1g.Memory)).Should(BeTrue())
		...
		When("component without class definition", func() {
			...
			It("should fail with classDefRef", func() {
				comp := &v1alpha1.ClusterComponentSpec{
					ComponentDefRef: compType2,
					ClassDefRef:     &v1alpha1.ClassDefRef{Class: apps.Class1c1gName},
				}
				_, err := clsMgr.ChooseClass(comp)
				Expect(err).Should(HaveOccurred())
	...
	It("get classes should succeed", func() {
		var (
			...
			specClassDefRef        = v1alpha1.ClassDefRef{Name: classDefinitionObjName, Class: apps.Class1c1gName}
			statusClassDefRef      = v1alpha1.ClassDefRef{Name: classDefinitionObjName, Class: "general-100c100g"}
			clsMgr                 *Manager
			compType               = "mysql"
		)

		classDef := apps.NewComponentClassDefinitionFactory(classDefinitionObjName, "apecloud-mysql", compType).
		...
		classDef.Status.Classes = []v1alpha1.ComponentClass{
			{
