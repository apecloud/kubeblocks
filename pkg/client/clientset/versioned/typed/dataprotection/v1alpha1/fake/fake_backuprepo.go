/*
Copyright (C) 2022-2023 ApeCloud Co., Ltd

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"
	json "encoding/json"
	"fmt"

	v1alpha1 "github.com/apecloud/kubeblocks/apis/dataprotection/v1alpha1"
	dataprotectionv1alpha1 "github.com/apecloud/kubeblocks/pkg/client/applyconfiguration/dataprotection/v1alpha1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeBackupRepos implements BackupRepoInterface
type FakeBackupRepos struct {
	Fake *FakeDataprotectionV1alpha1
}

var backupreposResource = v1alpha1.SchemeGroupVersion.WithResource("backuprepos")

var backupreposKind = v1alpha1.SchemeGroupVersion.WithKind("BackupRepo")

// Get takes name of the backupRepo, and returns the corresponding backupRepo object, and an error if there is any.
func (c *FakeBackupRepos) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.BackupRepo, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootGetAction(backupreposResource, name), &v1alpha1.BackupRepo{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.BackupRepo), err
}

// List takes label and field selectors, and returns the list of BackupRepos that match those selectors.
func (c *FakeBackupRepos) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.BackupRepoList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootListAction(backupreposResource, backupreposKind, opts), &v1alpha1.BackupRepoList{})
	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.BackupRepoList{ListMeta: obj.(*v1alpha1.BackupRepoList).ListMeta}
	for _, item := range obj.(*v1alpha1.BackupRepoList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested backupRepos.
func (c *FakeBackupRepos) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewRootWatchAction(backupreposResource, opts))
}

// Create takes the representation of a backupRepo and creates it.  Returns the server's representation of the backupRepo, and an error, if there is any.
func (c *FakeBackupRepos) Create(ctx context.Context, backupRepo *v1alpha1.BackupRepo, opts v1.CreateOptions) (result *v1alpha1.BackupRepo, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootCreateAction(backupreposResource, backupRepo), &v1alpha1.BackupRepo{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.BackupRepo), err
}

// Update takes the representation of a backupRepo and updates it. Returns the server's representation of the backupRepo, and an error, if there is any.
func (c *FakeBackupRepos) Update(ctx context.Context, backupRepo *v1alpha1.BackupRepo, opts v1.UpdateOptions) (result *v1alpha1.BackupRepo, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootUpdateAction(backupreposResource, backupRepo), &v1alpha1.BackupRepo{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.BackupRepo), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeBackupRepos) UpdateStatus(ctx context.Context, backupRepo *v1alpha1.BackupRepo, opts v1.UpdateOptions) (*v1alpha1.BackupRepo, error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootUpdateSubresourceAction(backupreposResource, "status", backupRepo), &v1alpha1.BackupRepo{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.BackupRepo), err
}

// Delete takes name of the backupRepo and deletes it. Returns an error if one occurs.
func (c *FakeBackupRepos) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewRootDeleteActionWithOptions(backupreposResource, name, opts), &v1alpha1.BackupRepo{})
	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeBackupRepos) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewRootDeleteCollectionAction(backupreposResource, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.BackupRepoList{})
	return err
}

// Patch applies the patch and returns the patched backupRepo.
func (c *FakeBackupRepos) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.BackupRepo, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceAction(backupreposResource, name, pt, data, subresources...), &v1alpha1.BackupRepo{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.BackupRepo), err
}

// Apply takes the given apply declarative configuration, applies it and returns the applied backupRepo.
func (c *FakeBackupRepos) Apply(ctx context.Context, backupRepo *dataprotectionv1alpha1.BackupRepoApplyConfiguration, opts v1.ApplyOptions) (result *v1alpha1.BackupRepo, err error) {
	if backupRepo == nil {
		return nil, fmt.Errorf("backupRepo provided to Apply must not be nil")
	}
	data, err := json.Marshal(backupRepo)
	if err != nil {
		return nil, err
	}
	name := backupRepo.Name
	if name == nil {
		return nil, fmt.Errorf("backupRepo.Name must be provided to Apply")
	}
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceAction(backupreposResource, *name, types.ApplyPatchType, data), &v1alpha1.BackupRepo{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.BackupRepo), err
}

// ApplyStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating ApplyStatus().
func (c *FakeBackupRepos) ApplyStatus(ctx context.Context, backupRepo *dataprotectionv1alpha1.BackupRepoApplyConfiguration, opts v1.ApplyOptions) (result *v1alpha1.BackupRepo, err error) {
	if backupRepo == nil {
		return nil, fmt.Errorf("backupRepo provided to Apply must not be nil")
	}
	data, err := json.Marshal(backupRepo)
	if err != nil {
		return nil, err
	}
	name := backupRepo.Name
	if name == nil {
		return nil, fmt.Errorf("backupRepo.Name must be provided to Apply")
	}
	obj, err := c.Fake.
		Invokes(testing.NewRootPatchSubresourceAction(backupreposResource, *name, types.ApplyPatchType, data, "status"), &v1alpha1.BackupRepo{})
	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.BackupRepo), err
}
