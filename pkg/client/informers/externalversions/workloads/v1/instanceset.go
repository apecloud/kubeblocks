	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.WorkloadsV1().InstanceSets(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.WorkloadsV1().InstanceSets(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *instanceSetInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&workloadsv1.InstanceSet{}, f.defaultInformer)
}

func (f *instanceSetInformer) Lister() v1.InstanceSetLister {
	return v1.NewInstanceSetLister(f.Informer().GetIndexer())
}
