...
// List lists all ServiceDescriptors in the indexer.
func (s *serviceDescriptorLister) List(selector labels.Selector) (ret []*v1.ServiceDescriptor, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ServiceDescriptor))
	})
	return ret, err
}
...
// List lists all ServiceDescriptors in the indexer for a given namespace.
func (s serviceDescriptorNamespaceLister) List(selector labels.Selector) (ret []*v1.ServiceDescriptor, err error) {
...
// Get retrieves the ServiceDescriptor from the indexer for a given namespace and name.
func (s serviceDescriptorNamespaceLister) Get(name string) (*v1.ServiceDescriptor, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("servicedescriptor"), name)
	}
	return obj.(*v1.ServiceDescriptor), nil
}
