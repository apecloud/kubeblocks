...
type ClusterTopologyLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ClusterTopology, err error)
	// Get retrieves the ClusterTopology from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ClusterTopology, error)
...
// List lists all ClusterTopologies in the indexer.
func (s *clusterTopologyLister) List(selector labels.Selector) (ret []*v1alpha1.ClusterTopology, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ClusterTopology))
	})
	return ret, err
}

// Get retrieves the ClusterTopology from the index for a given name.
func (s *clusterTopologyLister) Get(name string) (*v1alpha1.ClusterTopology, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("clustertopology"), name)
	}
	return obj.(*v1alpha1.ClusterTopology), nil
}
