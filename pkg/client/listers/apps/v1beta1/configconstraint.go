...
// List lists all ConfigConstraints in the indexer.
func (s *configConstraintLister) List(selector labels.Selector) (ret []*v1beta1.ConfigConstraint, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ConfigConstraint))
	})
	return ret, err
}

// Get retrieves the ConfigConstraint from the index for a given name.
func (s *configConstraintLister) Get(name string) (*v1beta1.ConfigConstraint, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("configconstraint"), name)
	}
	return obj.(*v1beta1.ConfigConstraint), nil
}
