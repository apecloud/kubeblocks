...
type ParametersDefinitionLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ParametersDefinition, err error)
	// Get retrieves the ParametersDefinition from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ParametersDefinition, error)
...
// List lists all ParametersDefinitions in the indexer.
func (s *parametersDefinitionLister) List(selector labels.Selector) (ret []*v1beta1.ParametersDefinition, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ParametersDefinition))
	})
	return ret, err
}

// Get retrieves the ParametersDefinition from the index for a given name.
func (s *parametersDefinitionLister) Get(name string) (*v1beta1.ParametersDefinition, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("parametersdefinition"), name)
	}
	return obj.(*v1beta1.ParametersDefinition), nil
}
