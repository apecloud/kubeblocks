...
type InstanceSetLister interface {
	...
	List(selector labels.Selector) (ret []*v1.InstanceSet, err error)
...
// List lists all InstanceSets in the indexer.
func (s *instanceSetLister) List(selector labels.Selector) (ret []*v1.InstanceSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.InstanceSet))
	})
	return ret, err
}
...
type InstanceSetNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.InstanceSet, error)
...
// List lists all InstanceSets in the indexer for a given namespace.
func (s instanceSetNamespaceLister) List(selector labels.Selector) (ret []*v1.InstanceSet, err error) {
...
// Get retrieves the InstanceSet from the indexer for a given namespace and name.
func (s instanceSetNamespaceLister) Get(name string) (*v1.InstanceSet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("instanceset"), name)
	}
	return obj.(*v1.InstanceSet), nil
}
