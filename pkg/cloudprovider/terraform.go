...
func (p *TFPlugin) Install() error {
	...
	if stat, err := os.Stat(pluginPath); err == nil {
		if stat.Size() > 0 {
			utils.Infof("Plugin %s has already exists, skip downloading", p.Source)
			return nil
	...
	utils.Infof("Downloading plugin %s", p.Source)
	// Create the file
...
func tfApply(template string, tfDir string, destroy bool) error {
	...
	utils.Infof("Execute terraform init: %s", strings.Join(args, " "))
	...
	utils.Infof("Execute terraform apply: %s", strings.Join(args, " "))
	if err := terraform.RunCli(args); err != nil {
