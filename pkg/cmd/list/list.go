...
func (c *Command) Build() *cobra.Command {
	o := newOptions(c.Streams, c.GroupKind)

	cmd := &cobra.Command{
		Use:   "list",
		Short: c.Short,
		Run: func(cmd *cobra.Command, args []string) {
			cmdutil.CheckErr(o.Complete(c.Factory, cmd))
...
func newOptions(streams genericclioptions.IOStreams, groupKind schema.GroupKind) *options {
	return &options{
...
func (o *options) Complete(f cmdutil.Factory, cmd *cobra.Command) error {
	...
	if strings.Contains(outputOption, "yaml") || strings.Contains(outputOption, "json") {
		o.ServerPrint = false
	}
...
func (o *options) Run(f cmdutil.Factory) error {
	r := f.NewBuilder().
		...
		ResourceTypeOrNameArgs(true, o.GroupKind.String()).
		ContinueOnError().
		Latest().
		Flatten().
