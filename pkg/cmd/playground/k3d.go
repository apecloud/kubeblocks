...
func (i *Installer) Uninstall() error {
	...
	err = utils.RemoveConfig(i.ClusterName)
	if err != nil {
...
func (i *Installer) SetKubeconfig() error {
	...
	return os.Setenv("KUBECONFIG", utils.ConfigPath(i.cfg.Cluster.Name))
}
...
func installCharts(i *Installer) error {
	install := func(cs []helm.InstallOpts) error {
		cfg, err := helm.NewActionConfig("", utils.ConfigPath(i.ClusterName))
		if err != nil {
			return err
		}
