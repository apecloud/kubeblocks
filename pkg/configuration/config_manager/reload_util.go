...
import (
	...
	gotemplate2 "github.com/apecloud/kubeblocks/pkg/gotemplate"
...
func wrapGoTemplateRun(ctx context.Context, tplScriptPath string, tplContent string, updatedParams map[string]string, formatConfig *appsv1alpha1.FormatterConfig, dataType string, dsn string) error {
	...
	values := gotemplate2.ConstructFunctionArgList(updatedParams)
	values[buildInFilesObjectName] = newFiles(filepath.Dir(tplScriptPath))
	engine := gotemplate2.NewTplEngine(&values, constructReloadBuiltinFuncs(ctx, commandChannel, formatConfig), tplScriptPath, nil, nil)
	_, err = engine.Render(tplContent)
	return err
}

func constructReloadBuiltinFuncs(ctx context.Context, cc DynamicParamUpdater, formatConfig *appsv1alpha1.FormatterConfig) *gotemplate2.BuiltInObjectsFunc {
	return &gotemplate2.BuiltInObjectsFunc{
		builtInExecFunctionName: func(command string, args ...string) (string, error) {
			execCommand := exec.CommandContext(ctx, command, args...)
			stdout, err := util.ExecShellCommand(execCommand)
		...
		builtInParamsPatchFunctionName: func(updatedParams map[string]string, basefile, newfile string) error {
			...
			newConfig, err := core2.ApplyConfigPatch(b, core2.FromStringPointerMap(updatedParams), formatConfig)
			if err != nil {
				return err
			}
...
func createUpdatedParamsPatch(newVersion []string, oldVersion []string, formatCfg *appsv1alpha1.FormatterConfig) (map[string]string, error) {
	...
	patch, err := core2.CreateMergePatch(&core2.ConfigResource{ConfigData: oldData}, &core2.ConfigResource{ConfigData: newData}, patchOption)
	if err != nil {
		return nil, err
	}

	params := core2.GenerateVisualizedParamsList(patch, formatCfg, nil)
	r := make(map[string]string)
	for _, key := range params {
		if key.UpdateType != core2.DeletedType {
...
func createFileRegex(fileRegex string) (regexFilter, error) {
	...
	if err != nil {
		return nil, core2.WrapError(err, "failed to create regexp [%s]", fileRegex)
	}
