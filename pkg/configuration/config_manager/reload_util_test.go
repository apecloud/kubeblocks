...
var _ = Describe("ReloadUtil Test", func() {
	...
	Context("TestReloadBuiltinFunctions", func() {
		It("test build-in exec", func() {
			engine := gotemplate2.NewTplEngine(nil,
...
`
			...
			values := gotemplate2.ConstructFunctionArgList(map[string]string{
				"key_buffer_size": "128M",
			})
			engine := gotemplate2.NewTplEngine(&values, constructReloadBuiltinFuncs(ctx, &mockCommandChannel{}, createIniFormatter("test")), "for_test", nil, nil)
		...
		It("test build-in patchParams", func() {
			...
			values := gotemplate2.ConstructFunctionArgList(params)
			engine := gotemplate2.NewTplEngine(&values, constructReloadBuiltinFuncs(context.TODO(), nil, createIniFormatter("test")), "for_test", nil, nil)
