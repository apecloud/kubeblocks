...
import (
	...
	unstructured2 "github.com/apecloud/kubeblocks/pkg/unstructured"
)
...
func init() {
	...
	loaderProvider[CfgRawType] = func(option CfgOption) (*cfgWrapper, error) {
		...
		meta := cfgWrapper{
			name:      "raw",
			fileCount: 0,
			v:         make([]unstructured2.ConfigObject, 1),
			indexer:   make(map[string]unstructured2.ConfigObject, 1),
		}

		v, err := unstructured2.LoadConfig(meta.name, string(option.RawData), option.CfgType)
		if err != nil {
	...
	loaderProvider[CfgCmType] = func(option CfgOption) (*cfgWrapper, error) {
		...
		meta := cfgWrapper{
			...
			fileCount: fileCount,
			v:         make([]unstructured2.ConfigObject, fileCount),
			indexer:   make(map[string]unstructured2.ConfigObject, 1),
		}

		var err error
		var index = 0
		var v unstructured2.ConfigObject
		for fileName, content := range ctx.ConfigData {
			if ctx.CMKeys != nil && !ctx.CMKeys.InArray(fileName) {
				continue
			}
			if v, err = unstructured2.LoadConfig(fileName, content, option.CfgType); err != nil {
...
type cfgWrapper struct {
	...
	indexer map[string]unstructured2.ConfigObject
	v       []unstructured2.ConfigObject
}
...
func (c *cfgWrapper) MergeFrom(params map[string]interface{}, option CfgOpOption) error {
	var err error
	var cfg unstructured2.ConfigObject
...
func (c *cfgWrapper) Query(jsonpath string, option CfgOpOption) ([]byte, error) {
	...
	return util2.RetrievalWithJSONPath(cfg.GetAllParameters(), jsonpath)
}
...
func (c *cfgWrapper) queryAllCfg(jsonpath string, option CfgOpOption) ([]byte, error) {
	...
	return util2.RetrievalWithJSONPath(tops, jsonpath)
}

func (c cfgWrapper) getConfigObject(option CfgOpOption) unstructured2.ConfigObject {
	if len(c.v) == 0 {
		return nil
	}
...
func (c *cfgWrapper) generateKey(paramKey string, option CfgOpOption) string {
	if option.IniContext != nil && len(option.IniContext.SectionName) > 0 {
		return strings.Join([]string{option.IniContext.SectionName, paramKey}, unstructured2.DelimiterDot)
	}
...
func flattenMap(m map[string]interface{}, prefix string) []ParameterPair {
	if prefix != "" {
		prefix += unstructured2.DelimiterDot
	}
	...
	for k, val := range m {
		...
		default:
			var v *string = nil
			if val != nil {
				v = util2.ToPointer(cast.ToString(val))
			}
