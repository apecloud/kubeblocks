...
func difference(base *cfgWrapper, target *cfgWrapper) (*ConfigPatchInfo, error) {
	fromOMap := util2.ToSet(base.indexer)
	fromNMap := util2.ToSet(target.indexer)

	addSet := util2.Difference(fromNMap, fromOMap)
	deleteSet := util2.Difference(fromOMap, fromNMap)
	updateSet := util2.Difference(fromOMap, deleteSet)
	...
	for elem := range updateSet.Iter() {
		...
		patch, err := util2.JSONPatch(old.GetAllParameters(), new.GetAllParameters())
		if err != nil {
			return nil, err
		}
