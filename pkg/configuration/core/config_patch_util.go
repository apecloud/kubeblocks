...
func LoadRawConfigObject(data map[string]string, formatConfig *appsv1alpha1.FormatterConfig, keys []string) (map[string]unstructured2.ConfigObject, error) {
	r := make(map[string]unstructured2.ConfigObject)
	cmKeySet := FromCMKeysSelector(keys)
...
func FromConfigObject(name, config string, formatConfig *appsv1alpha1.FormatterConfig) (unstructured2.ConfigObject, error) {
	configObject, err := unstructured2.LoadConfig(name, config, formatConfig.Format)
	if err != nil {
		return nil, err
	}
