...
func getUpdateParameterList(cfg *ConfigPatchInfo, trimField string) ([]string, error) {
	...
	walkFn := func(parent, cur string, v reflect.Value, fn util2.UpdateFn) error {
		if cur != "" {
			if parent != "" {
				cur = parent + "." + cur
			}
	...
	for _, diff := range cfg.UpdateConfig {
		...
		if err := util2.UnstructuredObjectWalk(updatedParams, walkFn, true); err != nil {
...
func IsUpdateDynamicParameters(cc *appsv1alpha1.ConfigConstraintSpec, cfg *ConfigPatchInfo) (bool, error) {
	...
	updateParams := util2.NewSet(params...)

	// if ConfigConstraint has StaticParameters, check updated parameter
	if len(cc.StaticParameters) > 0 {
		staticParams := util2.NewSet(cc.StaticParameters...)
		union := util2.Union(staticParams, updateParams)
		if union.Length() > 0 {
			return false, nil
		}
	...
	if len(cc.DynamicParameters) > 0 {
		dynamicParams := util2.NewSet(cc.DynamicParameters...)
		diff := util2.Difference(updateParams, dynamicParams)
