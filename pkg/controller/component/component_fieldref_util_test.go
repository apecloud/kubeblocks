...
var _ = Describe("ComponentRef Fields Tests", func() {
	...
	Context("test fieldRef", func() {
		var clusterDefBuilder *apps.MockClusterDefFactory
		var clusterBuilder *apps.MockClusterFactory

		BeforeEach(func() {
			By("create cluster definition builder")
			clusterDefBuilder = apps.NewClusterDefFactory(clusterDefName).
				AddComponentDef(apps.StatefulMySQLComponent, mysqlCompDefName).
				AddComponentDef(apps.StatefulMySQLComponent, referredCompDefName)

			// add one mysql component
			clusterDefBuilder = clusterDefBuilder.AddComponentDef(apps.StatefulMySQLComponent, mysqlCompDefName)

			By("create cluste builder")
			clusterBuilder = apps.NewClusterFactory(clusterNamespace, clusterName, clusterDefName, clusterVersionName)
		})
