...
func buildKBAgentStartupEnvs(synthesizedComp *SynthesizedComponent) ([]corev1.EnvVar, error) {
	...
	if synthesizedComp.LifecycleActions.ComponentLifecycleActions != nil {
		...
		if a, p := buildProbe4KBAgent(synthesizedComp.LifecycleActions.RoleProbe, "roleProbe", synthesizedComp.FullCompName); a != nil && p != nil {
			// Role-label recovery is owned by the workload role-event handler. Keep
			// roleProbe configuration free of PodTemplate-only file watchers so
			// enabling the recovery path does not force existing Pods to recreate.
			actions = append(actions, *a)
...
func hasActionDefined(synthesizedComp *SynthesizedComponent) bool {
	if synthesizedComp.LifecycleActions.ComponentLifecycleActions != nil || len(synthesizedComp.LifecycleActions.CustomActions) > 0 {
		return true
	}
