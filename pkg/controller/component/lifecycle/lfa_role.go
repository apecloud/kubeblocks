...
func (a *switchover) parameters(ctx context.Context, cli client.Reader) (map[string]string, error) {
	...
	m, err := hackParameters4Role(ctx, cli, a.synthesizedComp)
	if err != nil {
		return nil, err
	}
	if len(a.candidate) > 0 {
		m[switchoverCandidateName] = a.candidate
		m[switchoverCandidateFQDN] = component.PodFQDN(a.synthesizedComp.Namespace, a.synthesizedComp.FullCompName, a.candidate)
	}
	return m, nil
}
...
func hackParameters4Role(ctx context.Context, cli client.Reader, synthesizedComp *component.SynthesizedComponent) (map[string]string, error) {
	const (
	...
	role, err := leaderRole(synthesizedComp)
	if err != nil {
		return nil, err
	}
	...
	return map[string]string{
		...
		leaderPodFQDN: component.PodFQDN(namespace, synthesizedComp.FullCompName, pod.Name),
		leaderPodIP:   pod.Status.PodIP,
	}, nil
}

func leaderRole(synthesizedComp *component.SynthesizedComponent) (string, error) {
	targetRole := ""
