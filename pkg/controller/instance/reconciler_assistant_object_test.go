...
func TestAssistantObjectPreCondition(t *testing.T) {
...
func TestAssistantObjectReconcileEmpty(t *testing.T) {
	r := &assistantObjectReconciler{}
	inst := newTestInstance("cluster-component-0")
	tree := newTestTree(inst)
...
func TestAssistantObjectReconcileWithSharedObject(t *testing.T) {
	r := &assistantObjectReconciler{}
	inst := newTestInstance("cluster-component-0",
	...
	if obj == nil {
		t.Fatal("expected shared ConfigMap to be created in tree")
	}
}

func TestAssistantObjectReconcileWithOrdinalObject(t *testing.T) {
	r := &assistantObjectReconciler{}
	inst := newTestInstance("cluster-component-0",
		ordinalConfigMapAssistantObject("cluster-component-0"))
	tree := newTestTree(inst)
	...
	obj, err := tree.Get(ordinalConfigMap("cluster-component-0"))
	if err != nil {
		t.Fatalf("tree.Get() error = %v", err)
	}
	if obj == nil {
		t.Fatal("expected ordinal ConfigMap to be created in tree")
	}
}

func TestAssistantObjectReconcileWithNonCurrentOrdinalObject(t *testing.T) {
	r := &assistantObjectReconciler{}
	inst := newTestInstance("cluster-component-0",
		ordinalConfigMapAssistantObject("cluster-component-1"))
	tree := newTestTree(inst)
	...
	obj, err := tree.Get(ordinalConfigMap("cluster-component-1"))
	if err != nil {
		t.Fatalf("tree.Get() error = %v", err)
	}
	if obj != nil {
		t.Fatal("expected non-current ordinal ConfigMap to be skipped")
	}
}
...
func TestCheckObjectProvisionPolicy(t *testing.T) {
	...
	currentObj := ordinalConfigMap("cluster-component-0")
	result := r.checkObjectProvisionPolicy(inst, currentObj)
	if result == nil {
		t.Fatal("expected non-nil for current ordinal object")
	}

	otherObj := ordinalConfigMap("cluster-component-1")
	result = r.checkObjectProvisionPolicy(inst, otherObj)
	if result != nil {
		t.Fatal("expected nil for non-current ordinal object")
	}
}

func TestCreateOrUpdateOwnedCreatesNew(t *testing.T) {
	r := &assistantObjectReconciler{}
	inst := newTestInstance("cluster-component-0")
	obj := ordinalConfigMap("cluster-component-0")
	assistantObj := ordinalConfigMapAssistantObject(obj.Name)

	tree := newTestTree(inst)
	err := r.createOrUpdateOwned(tree, inst, assistantObj, obj)
	if err != nil {
		t.Fatalf("createOrUpdateOwned() error = %v", err)
	}

	result, err := tree.Get(obj)
	if err != nil {
		t.Fatalf("tree.Get() error = %v", err)
	}
	if result == nil {
		t.Fatal("expected object to be created in tree")
	}
}

func TestCreateOrUpdateOwnedUpdatesExisting(t *testing.T) {
	r := &assistantObjectReconciler{}
	inst := newTestInstance("cluster-component-0")
	existing := ordinalConfigMap("cluster-component-0")
	existing.Data = map[string]string{"old": "value"}
	obj := ordinalConfigMap("cluster-component-0")
	obj.Data = map[string]string{"new": "value"}
	assistantObj := ordinalConfigMapAssistantObject(obj.Name)
	...
	if result == nil {
		t.Fatal("expected object to exist in tree")
	}
	cm := result.(*corev1.ConfigMap)
	if cm.Data["new"] != "value" {
		t.Fatalf("expected data to be updated, got %#v", cm.Data)
	}
}
