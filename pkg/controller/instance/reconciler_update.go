...
func (r *updateReconciler) reconfigure(tree *kubebuilderx.ObjectTree, inst *workloads.Instance, pod *corev1.Pod) (bool, error) {
	...
	for _, config := range inst.Spec.Configs {
		if !r.isConfigUpdated(inst, config) {
			allUpdated = false
			if err := r.reconfigureConfig(tree, inst, pod, config); err != nil {
				return false, err
			}
		}
		r.setInstanceConfigStatus(inst, config)
	}
...
func (r *updateReconciler) setInstanceConfigStatus(inst *workloads.Instance, config workloads.ConfigTemplate) {
	if inst.Status.Configs == nil {
		inst.Status.Configs = make([]workloads.InstanceConfigStatus, 0)
	}
	status := workloads.InstanceConfigStatus{
		Name:       config.Name,
		Generation: config.Generation,
	}
	for i, configStatus := range inst.Status.Configs {
		if configStatus.Name == config.Name {
			inst.Status.Configs[i] = status
			return
		}
	}
	inst.Status.Configs = append(inst.Status.Configs, status)
}

func (r *updateReconciler) isConfigUpdated(inst *workloads.Instance, config workloads.ConfigTemplate) bool {
	for _, configStatus := range inst.Status.Configs {
		if configStatus.Name == config.Name {
