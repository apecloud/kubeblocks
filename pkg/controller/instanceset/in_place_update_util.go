...
func getPodUpdatePolicy(its *workloads.InstanceSet, pod *corev1.Pod) (podUpdatePolicy, workloads.PodUpdatePolicyType, error) {
	...
	// In case of the ITS is stopped and replicas is 0, we can't compose the instance template and instance
	...
	inst, err := buildInstanceByTemplateForUpdate(pod, templateList[index], its)
	if err != nil {
		return noOpsPolicy, "", err
	}
