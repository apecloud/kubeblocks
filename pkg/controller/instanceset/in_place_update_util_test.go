...
var _ = Describe("instance util test", func() {
	...
	Context("container image comparison", func() {
		It("defers the custom-image init command and tools image as one pair", func() {
			...
			defaultImagePod.Spec.Containers[1].Command = []string{kbagent.BinaryPath}
		...
		It("uses in-place policy for KB-managed tools image changes even when pod upgrade policy is ReCreate", func() {
			...
			oldPod.Spec.Containers = []corev1.Container{
				...
				{Name: "kbagent", Image: "docker.io/apecloud/kubeblocks-tools:1.0.0", Command: []string{kbagent.BinaryPath}},
			}
		...
		It("uses in-place policy for active config-manager tools image changes", func() {
			oldToolsImage := viper.GetString(constant.KBToolsImage)
			defer viper.Set(constant.KBToolsImage, oldToolsImage)
			viper.Set(constant.KBToolsImage, "docker.io/apecloud/kubeblocks-tools:1.0.0")
			...
			oldPod.Spec.Containers = []corev1.Container{
				{Name: "app", Image: "docker.io/apecloud/redis:7.2"},
				{Name: "config-manager", Image: "docker.io/apecloud/kubeblocks-tools:1.0.0", Command: []string{"/bin/config-manager"}},
			}
			oldPod.Spec.InitContainers = []corev1.Container{
				{Name: "install-config-manager-tool", Image: "docker.io/apecloud/kubeblocks-tools:1.0.0", Command: []string{"install"}},
			}
			newPod := oldPod.DeepCopy()
			newPod.Spec.Containers[1].Image = "mirror.local/apecloud/kubeblocks-tools:1.1.0"
			newPod.Spec.InitContainers[0].Image = "mirror.local/apecloud/kubeblocks-tools:1.1.0"
