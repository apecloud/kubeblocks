...
func (r *statusReconciler) setInstanceConfigStatus(its *workloads.InstanceSet) {
	if its.Status.InstanceStatus == nil {
		...
		for i := range its.Status.InstanceStatus {
			its.Status.InstanceStatus[i].Configs = configs
		}
	} else {
		...
		for i, newStatus := range its.Status.InstanceStatus {
			for _, status := range its.Status.InstanceStatus {
				if status.PodName == newStatus.PodName {
					if its.Status.InstanceStatus[i].Configs == nil {
						its.Status.InstanceStatus[i].Configs = make([]workloads.InstanceConfigStatus, 0)
					}
					for j, config := range status.Configs {
						if configs.Has(config.Name) {
							its.Status.InstanceStatus[i].Configs = append(its.Status.InstanceStatus[i].Configs, status.Configs[j])
						}
					}
					break
				}
