...
func (p *realUpdatePlan) planWalkFunc(vertex graph.Vertex) error {
	...
	if isPodUpdated {
		...
		if isRoleful && !intctrlutil.PodIsReadyWithLabel(*pod) {
			...
			if p.its.Spec.UpdateStrategy != nil && p.its.Spec.UpdateStrategy.MemberUpdateStrategy != nil {
				p.its.Spec.MemberUpdateStrategy = p.its.Spec.UpdateStrategy.MemberUpdateStrategy
			}
