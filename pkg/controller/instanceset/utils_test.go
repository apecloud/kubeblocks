...
var _ = Describe("utils test", func() {
	...
	Context("IsAllRequiredRolesExist", func() {
		It("works well", func() {
			its = builder.NewInstanceSetBuilder(namespace, name).
				SetRoles(roles).
				GetObject()
			...
			Expect(IsAllRequiredRolesExist(its)).Should(BeFalse())

			By("has leader")
			its.Status.MembersStatus[0].ReplicaRole = &roles[0]
			Expect(IsAllRequiredRolesExist(its)).Should(BeTrue())

			By("set two required roles")
			...
			its.Spec.Roles = roles
			By("only one required role presents")
			its.Status.MembersStatus = []workloads.MemberStatus{
				{
					PodName:     name + "-0",
					ReplicaRole: &roles[0],
			...
			By("all required roles present")
			its.Status.MembersStatus[1].ReplicaRole = &roles[1]
			Expect(IsAllRequiredRolesExist(its)).Should(BeTrue())
		})
	})
