...
func BuildActiveAllocations(tree *kubebuilderx.ObjectTree, its *workloads.InstanceSet) ([]Allocation, []string, error) {
	...
	for name, template := range nameMap {
		...
		allocations = append(allocations, Allocation{PodName: name, TemplateName: template.Name})
	}
	...
	templateNames = append(templateNames, instancetemplate.DefaultTemplateName)
	for name := range itsExt.InstanceTemplates {
		if name != instancetemplate.DefaultTemplateName {
...
func TemplateNameFromLabels(labels map[string]string) (string, bool, error) {
	if labels == nil {
		return "", false, nil
	}
	...
	if templateName, ok := labels[instancetemplate.TemplateNameLabelKey]; ok {
		return templateName, true, nil
	}
	return "", false, nil
}

// HistoricalTemplateHint resolves an old retained instance only from explicit allocation state or unambiguous non-flat naming.
func HistoricalTemplateHint(its *workloads.InstanceSet, podName string, knownTemplateNames []string) (string, bool, error) {
	parent, ordinal, ok := parseName(podName)
	if !ok {
		return "", false, nil
	}
...
func parseName(name string) (string, int, bool) {
