...
func TestIsActionFailure(t *testing.T) {
	tests := []struct {
		...
		{
			name: "waiting and failed",
			err:  newActionAggregateError([]error{ErrActionBusy, ErrActionFailed}),
			want: true,
		},
		{
			name: "in progress and timed out",
			err:  newActionAggregateError([]error{ErrActionInProgress, ErrActionTimedOut}),
			want: true,
		},
		{
			name: "waiting only",
			err:  newActionAggregateError([]error{ErrActionBusy, ErrActionInProgress, ErrPreconditionFailed}),
			want: false,
		},
		{
			name: "wrapped aggregate",
			err:  fmt.Errorf("wrapped: %w", newActionAggregateError([]error{ErrActionBusy, ErrActionFailed})),
			want: true,
		},
	}
