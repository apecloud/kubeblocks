...
func wrapGetEnvByName(templateBuilder *configTemplateBuilder, component *component2.SynthesizedComponent, localObjs []coreclient.Object) envBuildInFunc {
	wrapper := &envWrapper{
...
func (w *envWrapper) doEnvReplace(replacedVars *set.LinkedHashSetString, oldValue string, container *corev1.Container) (string, error) {
	var (
		...
		builtInEnvMap = component2.GetReplacementMapForBuiltInEnv(clusterName, clusterUID, componentName)
	)

	builtInEnvMap[constant.KBConnCredentialPlaceHolder] = component2.GenerateConnCredential(clusterName)
