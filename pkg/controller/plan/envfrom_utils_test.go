...
var _ = Describe("ConfigEnvFrom test", func() {
	...
	BeforeEach(func() {
		...
		cm := apps.NewCustomizedObj("config/envfrom-config.yaml", &corev1.ConfigMap{},
			testCtx.UseDefaultNamespace())

		configConstraint = apps.NewCustomizedObj("config/envfrom-constraint.yaml",
			&appsv1alpha1.ConfigConstraint{})

		clusterDef = apps.NewClusterDefFactory(clusterDefName).
			AddComponentDef(apps.StatefulMySQLComponent, mysqlCompDefName).
			AddConfigTemplate(cm.Name, cm.Name, configConstraint.Name, testCtx.DefaultNamespace, "mysql-config", apps.DefaultMySQLContainerName).
			GetObject()
		...
		origCMObject.Name = core2.GetComponentCfgName(clusterName, mysqlCompName, cm.Name)
	})
	...
	Context("test config template inject envfrom", func() {
		...
		It("should SyncEnvConfigmap success", func() {
			...
			k8sMockClient.MockPatchMethod(testutil.WithFailed(core2.MakeError("failed to patch"), testutil.WithTimes(1)),
