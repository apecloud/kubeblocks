...
func RenderConfigNScriptFiles(clusterVersion *appsv1alpha1.ClusterVersion,
	...
	if err := controllerutil.CreateOrUpdatePodVolumes(podSpec, renderWrapper.volumes); err != nil {
		return core2.WrapError(err, "failed to generate pod volume")
	}

	if err := buildConfigManagerWithComponent(podSpec, component.ConfigTemplates, ctx, cli, cluster, component); err != nil {
		return core2.WrapError(err, "failed to generate sidecar for configmap's reloader")
	}
...
func createConfigObjects(cli client.Client, ctx context.Context, objs []client.Object) error {
	for _, obj := range objs {
		if err := cli.Create(ctx, obj); err != nil {
			...
			if core2.IsSchedulableConfigResource(obj) {
				continue
			}
...
func buildConfigManagerWithComponent(podSpec *corev1.PodSpec, configSpecs []appsv1alpha1.ComponentConfigSpec,
	ctx context.Context, cli client.Client, cluster *appsv1alpha1.Cluster, component *component2.SynthesizedComponent) error {
	var err error
	var buildParams *configmanager.CfgManagerBuildParams

	volumeDirs, usingConfigSpecs := getUsingVolumesByConfigSpecs(podSpec, configSpecs)
	if len(volumeDirs) == 0 {
		return nil
	}
	configSpecMetas, err := configmanager.GetSupportReloadConfigSpecs(usingConfigSpecs, cli, ctx)
	if err != nil {
		return err
	}
	...
	configSpecMetas = configmanager.FilterSubPathVolumeMount(configSpecMetas, volumeDirs)
...
func checkAndUpdateSharProcessNamespace(podSpec *corev1.PodSpec, buildParams *configmanager.CfgManagerBuildParams, configSpecMetas []configmanager.ConfigSpecMeta) {
	shared := configmanager.NeedSharedProcessNamespace(configSpecMetas)
	if shared {
...
func updateEnvPath(container *corev1.Container, params *configmanager.CfgManagerBuildParams) {
	if len(params.ScriptVolume) == 0 {
		return
	}
	scriptPath := make([]string, 0, len(params.ScriptVolume))
	for _, volume := range params.ScriptVolume {
		if vm := configmanager.FindVolumeMount(params.Volumes, volume.Name); vm != nil {
			scriptPath = append(scriptPath, vm.MountPath)
		}
	}
	if len(scriptPath) != 0 {
		container.Env = append(container.Env, corev1.EnvVar{
			Name:  configmanager.KBConfigManagerPathEnv,
			Value: strings.Join(scriptPath, ":"),
		})
	}
}

func updateCfgManagerVolumes(podSpec *corev1.PodSpec, configManager *configmanager.CfgManagerBuildParams) {
	...
	for _, vm := range []*[]corev1.Volume{
		...
		for i := range *vm {
			podVolumes, _ = controllerutil.CreateOrUpdateVolume(podVolumes, (*vm)[i].Name, func(string) corev1.Volume {
				return (*vm)[i]
			}, nil)
		}
	}
	...
	for volumeName, volume := range configManager.ConfigLazyRenderedVolumes {
		usingContainers := controllerutil.GetPodContainerWithVolumeMount(podSpec, volumeName)
...
func getUsingVolumesByConfigSpecs(podSpec *corev1.PodSpec, configSpecs []appsv1alpha1.ComponentConfigSpec) ([]corev1.VolumeMount, []appsv1alpha1.ComponentConfigSpec) {
	...
	for _, configSpec := range configSpecs {
		usingContainers := controllerutil.GetPodContainerWithVolumeMount(podSpec, configSpec.VolumeName)
	...
	for _, configSpec := range usingConfigSpecs {
		...
		if !core2.NeedReloadVolume(configSpec) {
			continue
		}
		sets := cfgutil.NewSet()
		for _, container := range config2Containers[configSpec.Name] {
			volume := controllerutil.GetVolumeMountByVolume(container, configSpec.VolumeName)
...
func buildConfigManagerParams(cli client.Client, ctx context.Context, cluster *appsv1alpha1.Cluster, comp *component2.SynthesizedComponent, configSpecBuildParams []configmanager.ConfigSpecMeta, volumeDirs []corev1.VolumeMount, podSpec *corev1.PodSpec) (*configmanager.CfgManagerBuildParams, error) {
	cfgManagerParams := &configmanager.CfgManagerBuildParams{
		ManagerName:               constant.ConfigSidecarName,
		CharacterType:             comp.CharacterType,
		ComponentName:             comp.Name,
		SecreteName:               component2.GenerateConnCredential(cluster.Name),
		EnvConfigName:             component2.GenerateComponentEnvName(cluster.Name, comp.Name),
	...
	if err := configmanager.BuildConfigManagerContainerParams(cli, ctx, cfgManagerParams, volumeDirs); err != nil {
		return nil, err
	}
