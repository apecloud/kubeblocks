...
func buildComponentResources(reqCtx controllerutil.RequestCtx, cli client.Client,
	clusterDef *appsv1alpha1.ClusterDefinition,
	clusterVer *appsv1alpha1.ClusterVersion,
	cluster *appsv1alpha1.Cluster,
	component *component2.SynthesizedComponent) ([]client.Object, error) {
			...
			for _, cc := range []*[]corev1.Container{
				...
				for _, c := range *cc {
					for _, v := range c.VolumeMounts {
						...
						volumes, _ = controllerutil.CreateOrUpdateVolume(volumes, v.Name, func(volumeName string) corev1.Volume {
...
func addLeaderSelectorLabels(service *corev1.Service, component *component2.SynthesizedComponent) {
...
var _ = Describe("Cluster Controller", func() {
	...
	Context("with Deployment workload", func() {
		BeforeEach(func() {
			clusterDef = apps.NewClusterDefFactory(clusterDefName).
				AddComponentDef(apps.StatelessNginxComponent, nginxCompDefName).
				GetObject()
	...
	Context("with Stateful workload and with config template", func() {
		BeforeEach(func() {
			cm := apps.CreateCustomizedObj(&testCtx, "config/envfrom-config.yaml", &corev1.ConfigMap{},
				testCtx.UseDefaultNamespace())

			cfgTpl := apps.CreateCustomizedObj(&testCtx, "config/envfrom-constraint.yaml",
				&appsv1alpha1.ConfigConstraint{})

			configSpecName = cm.Name
			clusterDef = apps.NewClusterDefFactory(clusterDefName).
				AddComponentDef(apps.StatefulMySQLComponent, mysqlCompDefName).
				AddConfigTemplate(cm.Name, cm.Name, cfgTpl.Name, testCtx.DefaultNamespace, "mysql-config", apps.DefaultMySQLContainerName, "not-exist").
				GetObject()
			clusterVersion = apps.NewClusterVersionFactory(clusterVersionName, clusterDefName).
				AddComponentVersion(mysqlCompDefName).
				AddContainerShort("mysql", apps.ApeCloudMySQLImage).
	...
	Context("with Replications workload without pvc", func() {
		...
		BeforeEach(func() {
			clusterDef = apps.NewClusterDefFactory(clusterDefName).
				AddComponentDef(apps.ReplicationRedisComponent, redisCompDefName).
				AddComponentDef(apps.StatelessNginxComponent, nginxCompDefName).
				GetObject()
			clusterVersion = apps.NewClusterVersionFactory(clusterVersionName, clusterDefName).
				AddComponentVersion(redisCompDefName).
				AddContainerShort("redis", apps.DefaultRedisImageName).
				AddComponentVersion(nginxCompDefName).
				AddContainerShort("nginx", apps.NginxImage).
				GetObject()
			cluster = apps.NewClusterFactory(testCtx.DefaultNamespace, clusterName,
