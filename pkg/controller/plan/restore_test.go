...
var _ = Describe("PITR Functions", func() {
	...
	cleanEnv := func() {
		...
		apps.ClearResources(&testCtx, generics.PodSignature, inNS, ml, &opts)
	...
	Context("Test PITR", func() {
		...
		BeforeEach(func() {
			clusterDef = apps.NewClusterDefFactory(clusterDefName).
				AddComponentDef(apps.StatefulMySQLComponent, mysqlCompType).
				AddComponentDef(apps.StatelessNginxComponent, nginxCompType).
			...
			_ = apps.NewPodFactory(testCtx.DefaultNamespace, clusterName+"-"+mysqlCompName+"-0").
			...
			fullBackupTool = apps.CreateCustomizedObj(&testCtx, "backup/backuptool.yaml", &dpv1alpha1.BackupTool{}, apps.RandomizedObjName())
			...
			continuousBackupTool = apps.CreateCustomizedObj(&testCtx, "backup/pitr_backuptool.yaml",
				backupSelfDefineObj, apps.RandomizedObjName())
			// set datafile backup relies on logfile
			Expect(apps.ChangeObj(&testCtx, continuousBackupTool, func(tmpObj *dpv1alpha1.BackupTool) {
			...
			_ = apps.NewBackupPolicyTemplateFactory("backup-policy-template").
			...
			synthesizedComponent = &component.SynthesizedComponent{
				...
				Name:                  mysqlCompName,
				VolumeTypes:           []appsv1alpha1.VolumeTypeSpec{{Name: apps.DataVolumeName, Type: appsv1alpha1.VolumeTypeData}},
				Replicas:              1,
			}
			By("By creating remote pvc: ")
			remotePVC := apps.NewPersistentVolumeClaimFactory(
				testCtx.DefaultNamespace, "remote-pvc", clusterName, mysqlCompName, "log").
				SetStorage("1Gi").
				Create(&testCtx).GetObject()

			logfileRemotePVC := apps.NewPersistentVolumeClaimFactory(
			...
			logfileBackup := apps.NewBackupFactory(testCtx.DefaultNamespace, backupName).
		...
		It("Test restore", func() {
			...
			backupSnapshot := apps.NewBackupFactory(testCtx.DefaultNamespace, backupName).
				WithRandomName().
				SetBackupPolicyName("test-fake").
		...
		testPITR := func() {
			baseBackupPhysicalRestore := func() types.NamespacedName {
				...
				Eventually(apps.CheckObj(&testCtx, baseBackupJobKey, func(g Gomega, fetched *batchv1.Job) {
				...
				Eventually(apps.GetAndChangeObjStatus(&testCtx, baseBackupJobKey, func(fetched *batchv1.Job) {
			...
			baseBackupLogicalRestore := func() types.NamespacedName {
				...
				Eventually(apps.GetAndChangeObjStatus(&testCtx, logicJobKey, func(fetched *batchv1.Job) {
					fetched.Status.Conditions = []batchv1.JobCondition{{Type: batchv1.JobComplete}}
				})).Should(Succeed())
				return logicJobKey
			}
			...
			continuousPhysicalRestore := func() types.NamespacedName {
				...
				Eventually(apps.GetAndChangeObjStatus(&testCtx, jobKey, func(fetched *batchv1.Job) {
					fetched.Status.Conditions = []batchv1.JobCondition{{Type: batchv1.JobComplete}}
				})).Should(Succeed())
				return jobKey
			}
			...
			continuousLogicalRestore := func() types.NamespacedName {
				...
				Expect(apps.ChangeObj(&testCtx, continuousBackupTool, func(tool *dpv1alpha1.BackupTool) {
			...
			Eventually(apps.GetAndChangeObjStatus(&testCtx, client.ObjectKeyFromObject(cluster), func(fetched *appsv1alpha1.Cluster) {
			...
			for _, v := range backupJobKeys {
				Eventually(apps.CheckObjExists(&testCtx, v, &batchv1.Job{}, false)).Should(Succeed())
			}
		}
		...
		It("Test PITR restore when only support physical logical for full backup", func() {
			Expect(apps.ChangeObj(&testCtx, fullBackupTool, func(tool *dpv1alpha1.BackupTool) {
