...
func (c *configPatcher) Merge(baseData map[string]string, updatedData map[string]string) (map[string]string, error) {
	...
	configPatch, err := core2.TransformConfigPatchFromData(updatedData, formatter.Format, c.configSpec.Keys)
	if err != nil {
		return nil, err
	}
	...
	params := core2.GenerateVisualizedParamsList(configPatch, formatter, nil)
	for key, patch := range splitParameters(params) {
		...
		newConfig, err := core2.ApplyConfigPatch([]byte(v), patch, formatter)
		if err != nil {
			return nil, err
		}
		r[key] = newConfig
...
func NewTemplateMerger(template appsv1alpha1.LazyRenderedTemplateSpec, ctx context.Context, cli client.Client, builder *configTemplateBuilder, configSpec appsv1alpha1.ComponentConfigSpec, ccSpec *appsv1alpha1.ConfigConstraintSpec) (TemplateMerger, error) {
	...
	default:
		return nil, core2.MakeError("unknown template policy: %s", template.Policy)
...
func mergerConfigTemplate(template *appsv1alpha1.LazyRenderedTemplateSpec,
	...
	if configSpec.ConfigConstraintRef == "" {
		return nil, core2.MakeError("ConfigConstraintRef require not empty, configSpec[%v]", configSpec.Name)
	}
	...
	if err := cli.Get(ctx, ccKey, ccObj); err != nil {
		return nil, core2.WrapError(err, "failed to get ConfigConstraint, key[%v]", configSpec)
	}
	if ccObj.Spec.FormatterConfig == nil {
		return nil, core2.MakeError("importedConfigTemplate require ConfigConstraint.Spec.FormatterConfig, configSpec[%v]", configSpec)
	}
...
func splitParameters(params []core2.VisualizedParam) map[string]map[string]*string {
