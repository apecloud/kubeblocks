...
var _ = Describe("plan builder test", func() {
	Context("rsmWalkFunc function", func() {
		...
		BeforeEach(func() {
			...
			rsm = builder2.NewReplicatedStateMachineBuilder(namespace, name).
				AddFinalizers([]string{getFinalizer(&workloads.ReplicatedStateMachine{})}).
				GetObject()
		})

		It("should create object", func() {
			v := &model2.ObjectVertex{
				Obj:    rsm,
				Action: model2.ActionPtr(model2.CREATE),
			}
			k8sMock.EXPECT().
		...
		It("should update sts object", func() {
			...
			v := &model2.ObjectVertex{
				OriObj: stsOrig,
				Obj:    sts,
				Action: model2.ActionPtr(model2.UPDATE),
			}
			k8sMock.EXPECT().
		...
		It("should delete object", func() {
			v := &model2.ObjectVertex{
				Obj:    rsm,
				Action: model2.ActionPtr(model2.DELETE),
			}
			k8sMock.EXPECT().
				Update(gomock.Any(), gomock.Any(), gomock.Any()).
