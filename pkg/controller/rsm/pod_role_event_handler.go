...
func (h *PodRoleEventHandler) Handle(cli client.Client, reqCtx controllerutil.RequestCtx, recorder record.EventRecorder, event *corev1.Event) error {
...
func handleRoleChangedEvent(cli client.Client, reqCtx controllerutil.RequestCtx, recorder record.EventRecorder, event *corev1.Event) (string, error) {
	...
	name, _ := controllerutil.GetParentNameAndOrdinal(pod)
...
func parseProbeEventMessage(reqCtx controllerutil.RequestCtx, event *corev1.Event) *probeMessage {
