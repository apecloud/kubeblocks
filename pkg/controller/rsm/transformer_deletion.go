...
var _ graph2.Transformer = &ObjectDeletionTransformer{}

func (t *ObjectDeletionTransformer) Transform(ctx graph2.TransformContext, dag *graph2.DAG) error {
	transCtx, _ := ctx.(*rsmTransformContext)
	obj := transCtx.rsm
	if !model2.IsObjectDeleting(obj) {
		return nil
	}
	...
	ml := getLabels(obj)
	snapshot, err := model2.ReadCacheSnapshot(transCtx, obj, ml, deletionKinds()...)
	if err != nil {
		return err
	}
