...
var _ graph2.Transformer = &MemberReconfigurationTransformer{}
...
func (t *MemberReconfigurationTransformer) Transform(ctx graph2.TransformContext, dag *graph2.DAG) error {
	transCtx, _ := ctx.(*rsmTransformContext)
	if model2.IsObjectDeleting(transCtx.rsm) {
		return nil
	}
	rsm := transCtx.rsm
...
func cleanAction(transCtx *rsmTransformContext, dag *graph2.DAG) error {
	actionList, err := getActionList(transCtx, jobScenarioMembership)
	if err != nil {
		return err
	}
...
func deleteAction(transCtx *rsmTransformContext, dag *graph2.DAG, action *batchv1.Job) {
	cli, _ := transCtx.Client.(model2.GraphClient)
	doActionCleanup(dag, cli, action)
}

func createNextAction(transCtx *rsmTransformContext, dag *graph2.DAG, rsm *workloads.ReplicatedStateMachine, currentAction *batchv1.Job) error {
...
func getUnderlyingStsVertex(dag *graph2.DAG) (*model2.ObjectVertex, error) {
	vertices := model2.FindAll[*apps.StatefulSet](dag)
	if len(vertices) != 1 {
		return nil, fmt.Errorf("unexpected sts found, expected 1, but found: %d", len(vertices))
	}
	stsVertex, _ := vertices[0].(*model2.ObjectVertex)
