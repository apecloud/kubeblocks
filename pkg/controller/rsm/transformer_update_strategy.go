...
var _ graph2.Transformer = &UpdateStrategyTransformer{}

func (t *UpdateStrategyTransformer) Transform(ctx graph2.TransformContext, dag *graph2.DAG) error {
	transCtx, _ := ctx.(*rsmTransformContext)
	rsm := transCtx.rsm
	rsmOrig := transCtx.rsmOrig
	if !model2.IsObjectStatusUpdating(rsmOrig) {
		return nil
	}
	...
	graphCli, _ := transCtx.Client.(model2.GraphClient)
...
func doSwitchoverIfNeeded(transCtx *rsmTransformContext, dag *graph2.DAG, pods []corev1.Pod, podsToBeUpdated []*corev1.Pod) (bool, error) {
...
func createSwitchoverAction(dag *graph2.DAG, cli model2.GraphClient, rsm *workloads.ReplicatedStateMachine, pods []corev1.Pod) error {
