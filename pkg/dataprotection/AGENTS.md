...
- `errors/`: typed backup/restore error constructors and `ErrorType*` constants.
...
- Error constructors in `errors/` should use `intctrlutil.NewErrorf` with the existing `ErrorType*` constants so controllers can classify failures consistently.
