...
func (d *Deleter) doPreDeleteAction(
	...
	connectionEnv, err := d.buildConnectionEnv(backup)
	if err != nil {
		return nil, err
	}
...
func (d *Deleter) buildConnectionEnv(backup *dpv1alpha1.Backup) ([]corev1.EnvVar, error) {
	...
	return utils.BuildEnvByTarget(targetPod, target.ConnectionCredential, target.ContainerPort)
}
