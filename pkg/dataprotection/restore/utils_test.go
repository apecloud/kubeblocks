...
func TestBuildPVCVolumeAndMountShortensObservedOceanBasePrepareDataName(t *testing.T) {
	builder := &restoreJobBuilder{
		restore: &dpv1alpha1.Restore{ObjectMeta: metav1.ObjectMeta{Name: "restore"}},
		backupSet: BackupActionSet{
			Backup: &dpv1alpha1.Backup{ObjectMeta: metav1.ObjectMeta{Name: "backup"}},
	...
	claimName := "workdir-ob-pitr-p5n2-main-n3-v3-restore-oceanbase-0"
	identifier := "dp-claim-tpl"
	rawVolumeName := identifier + "-" + claimName
	if len(rawVolumeName) <= constant.KubeNameMaxLength {
		t.Fatalf("test setup should reproduce an overlength volume name, got length %d", len(rawVolumeName))
	}
	...
	if len(volume.Name) > constant.KubeNameMaxLength {
		t.Fatalf("volume name length = %d, want <= %d: %s", len(volume.Name), constant.KubeNameMaxLength, volume.Name)
	}
	if volume.Name == rawVolumeName {
		t.Fatalf("volume name was not shortened: %s", volume.Name)
	}
	if volume.Name != volumeMount.Name {
		t.Fatalf("volume name %q and volumeMount name %q should match", volume.Name, volumeMount.Name)
	}
	if volume.VolumeSource.PersistentVolumeClaim == nil || volume.VolumeSource.PersistentVolumeClaim.ClaimName != claimName {
		t.Fatalf("volume claim name = %q, want %q", volume.VolumeSource.PersistentVolumeClaim.ClaimName, claimName)
	}
}
