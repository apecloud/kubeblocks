...
func (c *Commands) ConnectCommand(info *engines.AuthInfo) []string {
	...
	var dsn = fmt.Sprintf("sqlplus sys/%s@//localhost:1521/%s as sysdba", userPass, serviceSID)
	if info != nil && info.UserPasswd != "" {
		dsn = fmt.Sprintf("sqlplus sys/%s@//localhost:1521/%s as sysdba", engines.AddSingleQuote(info.UserPasswd), serviceSID)
	}
