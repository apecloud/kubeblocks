...
func (hs horizontalScalingOpsHandler) getCompExpectReplicas(horizontalScaling opsv1alpha1.HorizontalScaling,
        ...
        if horizontalScaling.ScaleOut != nil {
                if horizontalScaling.ScaleOut.ReplicaChanges != nil {
                        compReplicas += *horizontalScaling.ScaleOut.ReplicaChanges
                }
                // When bringing offline instances back online, increment replicas
                if len(horizontalScaling.ScaleOut.OfflineInstancesToOnline) > 0 {
                        compReplicas += int32(len(horizontalScaling.ScaleOut.OfflineInstancesToOnline))
                }
        }
