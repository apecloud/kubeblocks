...
func TestHorizontalScalingParticipantAndProgressOrder(t *testing.T) {
	for _, fromBackup := range []bool{false, true} {
		want := []string{"workload(db)", "workload(db)"}
		attempts := 1
		if fromBackup {
		...
		for failAt := 0; failAt < attempts; failAt++ {
			t.Run(fmt.Sprintf("backup=%t/failAt=%d", fromBackup, failAt), func(t *testing.T) {
				...
				wantAction := []string(nil)
				if fromBackup {
					wantAction = []string{"names(db,1)", "names(db,1)"}
				} else {
...
// publishAllocation runs the workload revision/status producers explicitly.
// Tests decide when the owner publishes a new allocation; this does not create Pods.
...
func TestHorizontalScalingRestoreFailureAndCancellation(t *testing.T) {
	...
	t.Run("cancelling-does-not-access-backup", func(t *testing.T) {
		f := newHorizontalScalingFixture(t, scaleOutRequest("db", true))
		...
		if f.backupReads != 0 || f.restoreReads != 0 {
			t.Fatal("cancellation must not depend on backup availability")
		}
	})
}
...
// Cancelling must not restart restore or submit the forward target again.
func TestHorizontalScalingCancellingBackupKeepsSourceTarget(t *testing.T) {
	f := newHorizontalScalingFixture(t, scaleOutRequest("db", true))
	f.addBackup(t)
	hs := horizontalScalingOpsHandler{}
	if err := hs.Action(f.req, f.cli, f.res); err != nil {
	...
	f.replicas(t, "db", 1)
	reads, writes := f.backupReads+f.restoreReads, f.clusterWrites
	f.reconcile(t, opsv1alpha1.OpsSucceedPhase)
	f.replicas(t, "db", 1)
	if f.backupReads+f.restoreReads != reads || f.clusterWrites != writes {
		t.Fatal("cancellation resumed restore or resubmitted the forward target")
	}
	if f.res.OpsRequest.Status.Components["db"].Message == "Restore Data Completed" {
		t.Fatal("cancellation incorrectly reported restore completion")
	}
}
