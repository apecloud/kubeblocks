...
var _ = Describe("HorizontalScaling OpsRequest", func() {
	...
	Context("Test OpsRequest", func() {
		commonHScaleConsensusCompTest := func(reqCtx intctrlutil.RequestCtx,
			...
			hscaleValidatePolicy string) (*OpsResource, []*corev1.Pod) {
			...
			opsRes.OpsRequest = createHorizontalScaling(clusterName, horizontalScaling, hscaleValidatePolicy)
		...
		testHScaleReplicas := func(
			...
			opsRes, podList := commonHScaleConsensusCompTest(reqCtx, changeClusterSpec, horizontalScaling, constant.HscaleValidatePolicyStrict)
			mockHScale(podList)
		...
		testCancelHScale := func(
			...
			isScaleDown bool) {
			reqCtx := intctrlutil.RequestCtx{Ctx: testCtx.Ctx}
			opsRes, podList := commonHScaleConsensusCompTest(reqCtx, nil, horizontalScaling, constant.HscaleValidatePolicyStrict)
			var pod *corev1.Pod
			if isScaleDown {
				By("delete the pod")
				pod = podList[2]
		...
		testHScaleWithSpecifiedPod := func(changeClusterSpec func(cluster *appsv1.Cluster),
			...
			mockHScale func(podList []*corev1.Pod),
			hscaleValidatePolicy string) *OpsResource {
			reqCtx := intctrlutil.RequestCtx{Ctx: testCtx.Ctx}
			opsRes, podList := commonHScaleConsensusCompTest(reqCtx, changeClusterSpec, horizontalScaling, hscaleValidatePolicy)
		...
		createOpsAndToCreatingPhase := func(reqCtx intctrlutil.RequestCtx, opsRes *OpsResource, horizontalScaling opsv1alpha1.HorizontalScaling, policy string) *opsv1alpha1.OpsRequest {
			horizontalScaling.ComponentName = defaultCompName
			opsRes.OpsRequest = createHorizontalScaling(clusterName, horizontalScaling, policy)
		...
		It("test offline the specified pod but it is not online with the ignore policy", func() {
			By("init operations resources with CLusterDefinition/Hybrid components Cluster/consensus Pods")
			opsRes, _, _ := initOperationsResources(compDefName, clusterName)
			...
			}, constant.HscaleValidatePolicyIgnore)
			By("expect replicas not be changed")
			Eventually(testops.GetOpsRequestPhase(&testCtx, client.ObjectKeyFromObject(opsRes.OpsRequest))).Should(Equal(opsv1alpha1.OpsFailedPhase), fmt.Sprintf("info: %v", opsRes.OpsRequest))
		})
		...
		It("test offline the specified pod but it is not exist", func() {
			...
			unscalablePods := []string{offlineInsName}
			Expect(conditions[len(conditions)-1].Message).Should(ContainSubstring(
				fmt.Sprintf(`instances "%s" specified in the request is not exist`, strings.Join(unscalablePods, ", "))))
		})
...
func createHorizontalScaling(clusterName string, horizontalScaling opsv1alpha1.HorizontalScaling, policy string) *opsv1alpha1.OpsRequest {
	...
	ops.Annotations[constant.HscaleValidatePolicyKey] = policy
