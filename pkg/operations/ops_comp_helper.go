...
func (c componentOpsHelper) updateClusterComponentsAndShardings(cluster *appsv1.Cluster,
	...
	for index := range cluster.Spec.Shardings {
		...
		obj, selected := c.componentOpsSet[sharding.Name]
		if !selected { continue }
		if err := updateFunc(&sharding.Template, obj); err != nil { return err }
		if !hasInstanceTemplateTargets(obj) { continue }
		for i := range sharding.ShardTemplates {
			shardTemplate := &sharding.ShardTemplates[i]
			effective := sharding.Template
			if shardTemplate.Instances != nil { effective.Instances = shardTemplate.Instances }
			if shardTemplate.VolumeClaimTemplates != nil { effective.VolumeClaimTemplates = shardTemplate.VolumeClaimTemplates }
			if err := updateFunc(&effective, obj); err != nil { return err }
...
func hasInstanceTemplateTargets(obj ComponentOpsInterface) bool {
	switch v := obj.(type) {
	case opsv1alpha1.UpgradeComponent: return len(v.Instances) > 0
	case opsv1alpha1.VolumeExpansion: return len(v.Instances) > 0
