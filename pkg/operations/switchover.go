...
func doSwitchoverComponents(reqCtx intctrlutil.RequestCtx, cli client.Client, opsRes *OpsResource, switchoverList []opsv1alpha1.Switchover) error {
	var (
...
func handleSwitchoverProgress(reqCtx intctrlutil.RequestCtx, cli client.Client, opsRes *OpsResource) (int32, int32, int32, error) {
	...
	for _, switchover := range opsRequest.Spec.SwitchoverList {
		switchoverCondition := meta.FindStatusCondition(opsRes.OpsRequest.Status.Conditions, opsv1alpha1.ConditionTypeSwitchover)
		...
		checkJobProcessDetail := opsv1alpha1.ProgressStatusDetail{
		...
		if err = job.CheckJobSucceed(reqCtx.Ctx, cli, opsRes.Cluster, jobName); err != nil {
			if intctrlutil.IsTargetError(err, intctrlutil.ErrorTypeFatal) {
				...
				completedCount += 1
				failedCount += 1
				checkJobProcessDetail.Status = opsv1alpha1.FailedProgressStatus
				err = nil
			}
			checkJobProcessDetail.Message = fmt.Sprintf("switchover job %s is not succeed", jobName)
			setComponentSwitchoverProgressDetails(reqCtx.Recorder, opsRequest, appsv1.UpdatingClusterCompPhase, checkJobProcessDetail, switchover.ComponentName)
			continue
		} else {
			checkJobProcessDetail.Message = fmt.Sprintf("switchover job %s is succeed", jobName)
			checkJobProcessDetail.Status = opsv1alpha1.SucceedProgressStatus
			setComponentSwitchoverProgressDetails(reqCtx.Recorder, opsRequest, appsv1.UpdatingClusterCompPhase, checkJobProcessDetail, switchover.ComponentName)
		}

		// check the current component pod role label whether correct
		checkRoleLabelProcessDetail := opsv1alpha1.ProgressStatusDetail{
		...
		if errBuild != nil {
			...
			checkRoleLabelProcessDetail.Status = opsv1alpha1.FailedProgressStatus
			setComponentSwitchoverProgressDetails(reqCtx.Recorder, opsRequest, appsv1.UpdatingClusterCompPhase, checkRoleLabelProcessDetail, switchover.ComponentName)
			continue
		}
		consistency, err = checkPodRoleLabelConsistency(reqCtx.Ctx, cli, *synthesizedComp, &switchover, switchoverCondition)
		if err != nil {
		...
		if !consistency {
			...
			checkRoleLabelProcessDetail.Status = opsv1alpha1.SucceedProgressStatus
			setComponentSwitchoverProgressDetails(reqCtx.Recorder, opsRequest, appsv1.UpdatingClusterCompPhase, checkRoleLabelProcessDetail, switchover.ComponentName)
		}
		...
		componentProcessDetail := opsv1alpha1.ProgressStatusDetail{
		...
		setComponentSwitchoverProgressDetails(reqCtx.Recorder, opsRequest, appsv1.RunningClusterCompPhase, componentProcessDetail, switchover.ComponentName)
	}
	...
	if completedCount == expectCount {
		for _, jobName := range succeedJobs {
			if err := job.CleanJobByName(reqCtx.Ctx, cli, opsRes.Cluster, jobName); err != nil {
