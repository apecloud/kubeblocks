...
func ResolveParameterTemplate(cmpd appsv1.ComponentDefinitionSpec, configs []parametersv1alpha1.ComponentConfigDescription) []appsv1.ComponentFileTemplate {
	...
	for _, config := range cmpd.Configs {
		...
		if slices.Contains(tpls, config.Template) {
...
func updateConfigParameter(paramKey string,
	paramValue *string,
	parametersMap map[string]*ParameterMeta,
...
func resolveSchemaFromParametersDefinition(parametersDefs []*parametersv1alpha1.ParametersDefinition,
	templates []appsv1.ComponentFileTemplate,
	tpls map[string]*corev1.ConfigMap) (map[string]*ParameterMeta, error) {
	paramMeta := make(map[string]*ParameterMeta)
	mergeParams := func(params map[string]*ParameterMeta) {
	...
	for _, parameterDef := range parametersDefs {
		...
		mergeParams(ResolveConfigParameterSchema(parameterDef, configSpec))
	}
