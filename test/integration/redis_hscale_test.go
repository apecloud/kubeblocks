	...
	testReplicationRedisHorizontalScale := func() {
		...
		for _, sts := range stsList.Items {
			if strings.HasSuffix(sts.Name, strconv.Itoa(testapps.DefaultReplicationPrimaryIndex)) {
				Expect(sts.Labels[constant.RoleLabelKey]).Should(BeEquivalentTo(appsv1alpha1.ReplicationRolePrimary))
			} else {
				Expect(sts.Labels[constant.RoleLabelKey]).Should(BeEquivalentTo(appsv1alpha1.ReplicationRoleSecondary))
			}
		}
		...
		for _, sts := range stsList.Items {
			...
			if strings.HasSuffix(sts.Name, strconv.Itoa(testapps.DefaultReplicationPrimaryIndex)) {
				Expect(podList[0].Labels[constant.RoleLabelKey]).Should(BeEquivalentTo(appsv1alpha1.ReplicationRolePrimary))
			} else {
				Expect(podList[0].Labels[constant.RoleLabelKey]).Should(BeEquivalentTo(appsv1alpha1.ReplicationRoleSecondary))
			}
		}
	...
	Context("with Redis defined as replication Type and doing Horizontal scale", func() {
		BeforeEach(func() {
			...
			replicationRedisConfigVolumeMounts := []corev1.VolumeMount{
				{
					Name:      string(appsv1alpha1.ReplicationRolePrimary),
					MountPath: "/etc/conf/primary",
				},
				{
					Name:      string(appsv1alpha1.ReplicationRoleSecondary),
			...
			clusterDefObj = testapps.NewClusterDefFactory(clusterDefName).
				...
				AddConfigTemplate(primaryConfigName, primaryConfigName, "", testCtx.DefaultNamespace, string(appsv1alpha1.ReplicationRolePrimary)).
				AddConfigTemplate(secondaryConfigName, secondaryConfigName, "", testCtx.DefaultNamespace, string(appsv1alpha1.ReplicationRoleSecondary)).
